...
--ep ignite runs a graph-engine container compiled from the head-cut model
(every layer on the NPU) through ignite_xdna.pipelines.pose_pipeline, whose
decode is this script's numpy tail. --ingress numpy gives the container
npu.yolo's letterbox, the input the other EPs get; native (the default)
letterboxes straight into the NPU input plane. Its latency line then spans the
whole frame (letterbox, NPU, decode and NMS), not session.run and the decode.
...
def main():
    ...
    ap.add_argument("--ep", choices=["cpu", "npu", "ignite"], default="npu",
                    help="ignite: --model is a graph-engine .ignite container")
    ap.add_argument("--ingress", choices=["native", "numpy"], default="native",
                    help="--ep ignite only: native letterbox into the NPU input plane, or npu.yolo's letterbox")
    ...
    ep_name = f"ignite_{args.ingress}" if args.ep == "ignite" else args.ep
    cache_key = args.cache_key or YOLO_POSE_CUT_CACHE_KEY
    if args.fresh and args.ep != "ignite":
    ...
    if args.ep == "ignite":
        ...
        pipe = PosePipeline(args.model, conf_thres=args.conf, iou_thres=args.iou, max_det=args.max_det,
        ...
        print(f"model: graph-engine container (task {pipe.session.task}), every layer on the NPU, "
              f"numpy decode, ingress {args.ingress}, input {imgsz}x{imgsz}")
    else:
    ...
    try:
        for k, iid in enumerate(img_ids):
            ...
            if pipe is not None:
                ...
                people, _ = pipe.predict_sync(img, conf_thres=args.conf, iou_thres=args.iou)
                times.append(time.perf_counter() - t0)
                found = [(d.x0, d.y0, d.w, d.h, d.score, d.keypoints) for d in people]
            else:
                ...
                times.append(time.perf_counter() - t0)
                found = yp.postprocess(out, pad, scale, args.conf, args.iou, args.max_det)
            for x0, y0, w, h, s, kpts in found:
                flat = kpts.copy()
    ...
    what = "  (letterbox, NPU, decode and NMS)" if pipe is not None else ""
    print(f"\n{len(dets)} detections over {len(times)} images in {wall:.0f}s")
    print(f"inference  mean {times.mean():.2f} ms   median {np.median(times):.2f} ms{what}")

    RESULTS.mkdir(parents=True, exist_ok=True)
    out_json = args.dets or str(RESULTS / f"dets_kpts_{stem}_{ep_name}.json")
    ...
    print(f"\n=== {stem} | {ep_name.upper()} | {imgsz}px | {len(img_ids)} images ===")
    print(f"OKS mAP@50-95  {ev.stats[0] * 100:.2f}")
    print(f"OKS mAP@50     {ev.stats[1] * 100:.2f}")
    print(f"latency        {times.mean():.2f} ms{what}")
