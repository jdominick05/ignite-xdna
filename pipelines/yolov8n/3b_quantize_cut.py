"""
YOLO step 3b: quantize the head-cut model (from 1b_cut_head.py) to XINT8.

Difference from 3_quantize.py: there is no float tail left in the graph, so
there is nothing to exclude. `subgraphs_to_exclude` is gone and every node in
the model is a candidate for the NPU.

    conda activate resnet_env
    python pipelines/yolov8n/3b_quantize_cut.py --calib-dir data/coco_calib
    python pipelines/yolov8n/3b_quantize_cut.py --calib-dir data/coco_calib --adaround

Writes models/yolov8n_cut_xint8.onnx (or _xint8_adaround.onnx).
"""
import argparse
import copy
import glob
...
from npu.yolo import HEAD_OUTS, input_size, letterbox

IN_MODEL = MODELS / "yolov8n_cut.onnx"

# AMD's own YOLOv8 AdaRound params (CNN-examples/object_detection/yolov8m)
ADAROUND = {
...
class CocoCalibReader(CalibrationDataReader):
    """Uses npu.yolo.letterbox, the same transform inference uses. Do not
...
def main():
    ...
    # Quark's legacy QuantizationConfig is a dataclass, so a misspelled or
    # removed attribute assigns silently. Quark 0.12 also ships a new QConfig
    # whose fields differ; if get_default_config ever starts returning that,
    # every assignment below would become a no-op. Fail loudly instead.
