...
| [tnzr_bf16_32x32x32_repro_desktop2_20260923T0518Z.log](tnzr_bf16_32x32x32_repro_desktop2_20260923T0518Z.log) | MEASURED: the reference's hand-scheduled `.s`, fetched to `scratch/` (never vendored: that repo has no licence), assembled by this repo's Peano and linked by aiecc v1.4.2. **397.5 GFLOPS** on one tile in default pmode, 86.3% of 460.8, 99.9% of the reference. This is the first hand-written kernel executed on the NPU from this repo. `tools/tnzr_repro.py` |
| [l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log](l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log) | MEASURED, one sitting, same harness, banks and 32³ tile, every output exact: the reference's hand `.s` costs 282.0 cycles/call over an empty control (its 288 static bundles less the control's 6, so zero stalls), 391.5 GFLOPS. Upstream `mm.cc` costs 879.3 cycles (bf16, 28.5% of peak) and 514.1 (int8, 24.0%). `tools/l1_tile_bench.py` |
| [l1_tile_compiler_vs_hand_desktop2_20260923T0527Z.log](l1_tile_compiler_vs_hand_desktop2_20260923T0527Z.log), [l1_tile_empty_indexloop_desktop2_20260923T0530Z.log](l1_tile_empty_indexloop_desktop2_20260923T0530Z.log) | The first sitting, with a 64-bit loop counter and no control: 5.5–6.1 cycles/call slower each, which is exactly that counter's measured extra cost (25.4 vs 19.3 cycles/call) |
| [l1_tile_mm_k64_desktop2_20260923T0533Z.log](l1_tile_mm_k64_desktop2_20260923T0533Z.log), [l1_tile_mm_k128_desktop2_20260923T0533Z.log](l1_tile_mm_k128_desktop2_20260923T0533Z.log) | `mm.cc` K sweep (M = N = 32): bf16 36.1% / 41.3% of peak, int8 36.7% / 50.7%. The marginal cost per inner iteration is the static loop plus one cycle, one same-bank paired load per iteration |
| [l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log](l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log) | `mm.cc` at the bf16 array GEMM's own 64³ tile, no movement: 173.9 GFLOPS (37.7%) bf16, 362.3 GOPS (39.3%) int8. The array's 64/64/64 points at 2048³ and above (33.6–36.0%) are 89–95% of this, so the kernel's schedule is the "remaining two thirds" |
| [l1_tile_mm_direct_32x64x32_desktop2_20260923T0538Z.log](l1_tile_mm_direct_32x64x32_desktop2_20260923T0538Z.log) | Direct-mode cross-check: bf16 equals copy mode (1418.6 vs 1418.5 cycles); int8 is 7% slower (745.4 vs 697.1) with C beside the stack in bank 0 (attributed in the next row) |
| [l1_tile_mm_i8_k64_c_bank0_desktop2_20260923T0545Z.log](l1_tile_mm_i8_k64_c_bank0_desktop2_20260923T0545Z.log) | MEASURED, **a same-bank load+store in one bundle costs +1 cycle**. Copy mode with C in bank 0 reproduces direct mode (745.6). `mm.cc` int8 spills accumulators to the stack, and 12 epilogue bundles per block pair a stack reload with a C store: 4 blocks × 12 = 48 predicted, 48.5 measured. bf16 has none and does not move. `aie_bank_check.py` cannot see this (it ignores the stack and load+store pairs) |
| [vmac_chain_forwarding_desktop2_20260923T0602Z.log](vmac_chain_forwarding_desktop2_20260923T0602Z.log) | MEASURED, probe P2, **MAC forwarding distance**. Generated `.s` (`tools/l1_tile_bench.py`, kernels `chain_<bf16\|i8>_d<d>`): 8 dependent MACs on one accumulator, `d` bundles apart. All 8 land only at d ≥ 4 (bf16 `vmac.f`) and d ≥ 2 (int8 `vmac`), Peano's own spacing. Closer, bf16 lands 2/4/4 of 8 at d = 1/2/3 and int8 4 of 8 at d = 1, a stale-read pattern. No variant stalls: each costs the bundles it executes over the empty control, within 0.3 cycles |
...
- **"Device equals emulator" in `sesr_m7_bf16_silicon_smoke_desktop2_20260922.log`,
  `sesr_m7_bf16_silicon_desktop2_20260922.log` and `sesr_m7_bf16_native_host_desktop2_20260922.log` is scoped to
  the plain XINT8 weights.** Those logs stand as measured. On the AdaRound weights, with a byte-identical
  instruction stream, the device departs from the emulator on 4 of 6 inputs
  (`sesr_m7_adaround_bf16_silicon_desktop2_20260923.log`, reproduced by `sesr_m7_adaround_bf16_dump_desktop2_20260923.log`).
  Why is unexplained. With every tensor resident (`sesr_m7_adaround_bf16_noreuse_dump_desktop2_20260923.log`)
  it reaches all six inputs, starting in body.1, and none of the four candidate accumulate models reproduces it.
  A fifth, `exp_sum`, reproduces it (`sesr_m7_adaround_bf16_noreuse_replay_exp_sum_desktop2_20260923.log`), but it
  was fitted to those nine values and is unconfirmed on the core.
  Those three logs' "exact" also covers only the tensors resident at frame end.
...
| Log | Outcome |
|---|---|
| [program_ram_epilogue_census_phoenix_20260918T040616Z.log](program_ram_epilogue_census_phoenix_20260918T040616Z.log) | Per-opcode .text census of the persistent conv engine (tools/engine_opcode_census.py, Peano object-only): baseline 13,424 B; OP_MUL 13,648 (+224), OP_SCALE 13,888 (+464), OP_POOL 14,304 (+880), all three 15,280 B (+1,856) with accumulator stack moves 0 and 1,104 B still free of 16,384 (superseded 2026-09-21: that is the object; the linked core has 224 B free, see engine_linked_program_size_desktop2_20260921.log) - [BENCHMARKS](../../docs/BENCHMARKS.md#three-epilogue-opcodes-under-the-program-ram-budget-elementwise-mul-per-channel-scale-and-5x5-average-pool-bit-exact-on-silicon-2026-09-18-desktop-2) |
...
| [arch_compat_current_models_20260920.log](arch_compat_current_models_20260920.log) | Node census of the detection families ultralytics 8.4.142 ships, scored against the engine's op vocabulary. YOLO26 is the newest present (no YOLO27 in this toolchain) and the most compatible: zero unsupported conv shapes and a non-decode remainder of MatMul 4 / Softmax 2, one attention block, the same carve YOLO11n already gets. YOLO12n needs eight 7x7 convs and 16 MatMul / 9 Softmax. STRUCTURAL only, random weights, nothing lowered - [MODEL_ZOO](../../docs/MODEL_ZOO_BENCHMARKS.md#which-current-detection-architectures-could-lower-at-all-2026-09-20-desktop-2) |
| [yolo26n_bringup_20260920.log](yolo26n_bringup_20260920.log) | YOLO26n on the engine: it lowers (107 layers, 2 host), compiles (12.27 MB, 3 NPU segments) and dispatches on Device 0 at 9.009 ms NPU + 1.413 ms host - and is NOT correct: 38/107 layers exact, with every layer up to the qkv convolution EXACT and the first mismatch at the HOST attention segment itself, so the NPU side is clean and the carved attention core is not. Gates all passed (8 grouped convs all depthwise, no map under the 20-pixel tile). YOLO26 has NO DFL - 4-channel box branch, both Softmaxes attention - so it will also need a new decoder. No accuracy, no AMD comparison, NOT LANDED || [yolo26n_exact_20260920.log](yolo26n_exact_20260920.log) | YOLO26n is BIT-EXACT on the engine: 107/107 layers on Device 0, band-packed and plane-major alike, with both attention cores carved to the host (NPU 8.963 / 9.319 ms, host 1.342 / 1.380). Supersedes yolo26n_bringup_20260920.log's 38/107 verdict, which blamed the attention carve: the fault was a band-packed-layout regression - HostStep stages through a plane-major reshape, so banding a host segment's OUTPUT wrote right values to wrong addresses. Host-written tensors are now excluded from banding; YOLO11n re-checks 84/84. No accuracy, no decoder, no AMD comparison - [MODEL_ZOO](../../docs/MODEL_ZOO_BENCHMARKS.md#yolo26n-is-bit-exact-on-the-engine-2026-09-20-desktop-2) |
| [yolo26n_detections_20260920.log](yolo26n_detections_20260920.log) | YOLO26n DETECTS on the NPU. npu/yolo26_decode.py is the DFL-free anchor decode, checked against the float export's own output0 (box channels bit-exact, class 1.4e-07). On bus.jpg the container returns 6 detections identical to the XINT8 CPU reference - bus @0.924, four people, one low-confidence false positive the quantizer also makes on CPU. A stale container built before the host-banding fix first gave 2, which is the trap to remember. No mAP, no latency, no AMD comparison - [MODEL_ZOO](../../docs/MODEL_ZOO_BENCHMARKS.md#yolo26n-detects-on-the-npu-2026-09-20-desktop-2) |
| [yolo26n_vs_amd_20260920.log](yolo26n_vs_amd_20260920.log) | YOLO26n glass to glass in one sitting, arms alternating, 50 warm-up and 500 timed frames: AMD 43.099 / 43.118 ms, the engine 30.124 / 30.260 - 12.92 ms and 1.43x, and 1.53x on the network alone. AMD's EP places 12 of 1,526 nodes where the engine runs 107 of 107 layers with the two attention cores on the host, the same collapse YOLO11n shows. NOT an Ignition number and no badge may carry it: Ignition has no YOLO26 decode path. Both host tails are numpy, and the engine arm pays one readback per head, so 1.43x is a floor. No mAP - [MODEL_ZOO](../../docs/MODEL_ZOO_BENCHMARKS.md#yolo26n-against-amds-stack-143x-glass-to-glass-because-attention-stops-the-vitis-ai-ep-2026-09-20-desktop-2) |
| [decode_native_regmax_20260920.log](decode_native_regmax_20260920.log) | The native decoder implements a second box form: at reg_max 1 the four box channels ARE the four distances, so a DFL-free head decodes in C instead of numpy. Each head gained a box_val[256] table from the same _dequantized_logits helper the numpy path uses, which is what makes them bit-exact rather than close; the channel-blocked variant reads lanes 0-3 of block 0, not the block pair DFL needs. ABI 2 -> 3, and for_grid now refuses an unimplemented reg_max so the pipeline stops restating the rule. 0 mismatches over 2,000 stress trials at reg_max 16, 0 over 400 random-tensor trials at each of reg_max 1 and 16, and identical detections on the YOLO26n container; pipeline postprocess 0.221 -> 0.102 ms. CORRECTS the 3.49 ms decode in yolo26n_vs_amd_20260920.log, which was the benchmark's own full-tensor numpy decode and never the pipeline's - and names that benchmark's 9.13 ms numpy quantize, 30.5 % of its frame, as the real cost to fix - [BENCHMARKS](../../docs/BENCHMARKS.md#the-native-decoder-learns-what-reg_max-means-and-a-benchmarks-decode-was-never-the-pipelines-2026-09-20-desktop-2) |
| [bench_native_ingress_20260921.log](bench_native_ingress_20260921.log) | bench_container_vs_amd.py was filling the container's input plane with numpy - letterbox to float32 NCHW, quantize_input over 1,228,800 floats, transpose - while YoloPipeline ships an AVX2 ingress that does all of it in one pass. Worse, the quantize sat inside the arm's `forward` bucket, so it inflated what the tool called the engine's NETWORK time. The engine arm now shares AMD's cv2 letterbox and hands the square canvas to preprocess_to_plane; npu.yolo.letterbox was split into letterbox_canvas plus the float half so the transform stays in one place. THE PLANE BYTES ARE IDENTICAL either way (6/6 frames offline), so this is host cost and not a different frame - the LUT only equals the numpy quantize because the input scale is a power of two, and the tool now asserts that and refuses a container that fails it. Three arms alternating, 500 frames: YOLOv8n 29.452 -> 17.072 ms (42.0 %), YOLOv8s 38.798 -> 26.886 (30.7 %), detections identical, AMD 16.749 / 22.969 unchanged. THE ENGINE DID NOT GET FASTER and these are NOT badge numbers: decode is numpy on all three arms and the engine arm still reads back each head separately. Makes yolo26n_vs_amd_20260920.log's 1.43x an UNDERSTATEMENT, not re-measurable because that model's ONNX was lost with a removed worktree - [BENCHMARKS](../../docs/BENCHMARKS.md#the-benchmark-was-charging-the-engine-arm-12-ms-of-numpy-its-own-pipeline-never-pays-2026-09-21-desktop-2) |
| [bench_shipped_readback_20260921.log](bench_shipped_readback_20260921.log) | RETRACTS this directory's own claim that the per-head numpy dequantize was "the largest remaining host cost" at ~3.4 ms: that figure subtracted a dispatch measured in a DIFFERENT tool and configuration from this tool's forward, which measures nothing. Decomposed in one place on yolov8s: dispatch 17.375 ms of a 19.958 ms forward (87 %), readback+dequantize 2.018, of which read_heads alone is 0.271 and the float32 dequantize 1.212. NEGATIVE RESULT: a 256-entry dequantize LUT is SLOWER than astype+multiply, 2.131 vs 1.212 ms. The engine arm was moved from the read_tensor DEBUG helper (a sync, a copy and a numpy transpose per head) to the shipped read_heads anyway, mapping ONNX heads to layout heads on (scale, channels, height, width) and refusing loudly if that is not one-to-one; heads bit-identical 8/8 over two models and four frames. Sitting: yolov8n 17.230 -> 17.158 ms (spread 0.057), yolov8s 27.240 -> 27.075 (spread 0.291, WITHIN the noise). NIL ON SPEED, kept for representativeness. Method note: an isolated probe that replayed the readback without re-dispatching overstated the saving 5x (0.48 vs 0.05 ms) - [BENCHMARKS](../../docs/BENCHMARKS.md#the-readback-lever-is-worth-almost-nothing-and-the-claim-that-it-was-the-big-one-was-ours-2026-09-21-desktop-2) |
| [yolo26n_requant_vs_amd_20260921.log](yolo26n_requant_vs_amd_20260921.log) | YOLO26n re-derived from scratch because its quantized ONNX was lost with a removed worktree, then re-measured with the corrected benchmark. The re-derivation REPRODUCES the original: 364 cut nodes, identical op census, 107 layers / 2 host / 1725 rounds / 11.29 MB packets, insts.bin the SAME SIZE (510,612 B), 107/107 layers exact on Device 0. The cut was checked against the float export's own end-to-end output first - max corner delta 0.000 px - which is what catches a wrong head ORDER. Two generalizations, neither model-specific: cut_detect_head's tail vocabulary gains GatherElements and Mod (an NMS-free head top-k's in the graph), and head_name_for accepts the one2one_ prefix an end-to-end detector's inference branch carries. Sitting, three arms alternating, same container on both engine arms: AMD 44.039 ms, engine through the OLD tool 30.434 (1.45x, replicating the earlier 1.43x to 0.8 %), engine through the FIXED tool 17.414 - 2.53x, 6 detections on every arm, host tax 13.020 ms. SUPERSEDES yolo26n_vs_amd_20260920.log's 1.43x, which stands as measured. THE ENGINE DID NOT GET FASTER. No mAP, still not an Ignition number - [BENCHMARKS](../../docs/BENCHMARKS.md#yolo26n-re-derived-and-253x-amd-once-the-benchmark-stops-charging-it-13-ms-of-numpy-2026-09-21-desktop-2) |
| [yolo26n_coco_map_20260921.log](yolo26n_coco_map_20260921.log) | The accuracy number YOLO26 never had, on the FULL val2017 5,000 at conf 0.001: engine 23.74 mAP@50-95 / 39.40 @50, AMD's EP on the same XINT8 ONNX 23.64 / 39.16, and the FLOAT cut model on ORT CPU 39.66 / 55.87. So the engine and AMD agree to 0.10 mAP - expected for a layer-exact container, and it puts the loss in the QUANTIZATION RECIPE rather than either accelerator - while XINT8 costs 15.92 mAP against the model's own float export, a 40 % relative drop. The speed result (2.53x vs AMD) stands and must carry this number with it. Leading suspect is the Sigmoid -> HardSigmoid substitution (float graph Sigmoid 87, XINT8 graph HardSigmoid 87), already measured at 65-75 % of zoo XINT8 loss - but NOT ISOLATED here, because --silu-sigmoid refuses: "silu_sigmoid cannot be combined with host_regions", and YOLO26's two attention blocks are exactly what force host regions. The repo's one accuracy lever is structurally unavailable to this model, as for YOLO11n and YOLO-World. The harness is sound: the float arm's 39.66 uses the same cut, decode and scoring. 5_eval_map.py gained --decoder and --head-order so it is no longer YOLOv8-only - [BENCHMARKS](../../docs/BENCHMARKS.md#yolo26n-coco-map-253x-faster-than-amd-and-1592-map-poorer-than-its-own-float-model-2026-09-21-desktop-2) |
| [yolo26n_silu_recovery_20260921.log](yolo26n_silu_recovery_20260921.log) | The 15.92 mAP loss ISOLATED and then mostly fixed. tools/swap_activation.py puts Quark's HardSigmoid(alpha=1/6) into the FLOAT graph and changes nothing else, separating the activation's share from the arithmetic's: float+Sigmoid 39.66, float+HardSigmoid 29.55 (-10.11, 63.5 %), int8+HardSigmoid 23.74 (-5.81). The --silu-sigmoid epilogue was refused by an OVER-BROAD guard: the real conflict is a host region containing a SiLU, and YOLO26's attention cores contain none - reference_model rewrites 348 nodes and BOTH regions diff to zero overlap, zero bytes. The guard is now that check, regression-tested both ways on YOLO11n (/model.10/ block still refuses, attention core builds). Result: 107/107 layers exact against silu_sigmoid.reference_model, and 32.55 mAP - PREDICTED 33.85 before measuring, 1.3 short being the four-line fit's error. Same-sitting cost: 2.53x -> 2.49x AMD, +0.260 ms, so 8.81 mAP for 1.5 % of the frame. int8+sigmoid4 (32.55) BEATS float+HardSigmoid (29.55). HEADLINE PAIR: 2.49x AMD at 32.55 mAP against float's 39.66 - [BENCHMARKS](../../docs/BENCHMARKS.md#the-yolo26n-accuracy-loss-split-in-two-and-881-map-of-it-came-back-for-026-ms-2026-09-21-desktop-2) |
| [yolo26n_ignition_sitting_20260921.log](yolo26n_ignition_sitting_20260921.log) | The first YOLO26 number that is an IGNITION number rather than the container benchmark's. live_ignition.py in its balanced default against tools/amd_vitisai_yolo.py, arms alternating, two rounds, 50 warm-up and 500 timed frames of bus.jpg, xrt-smi idle before every run: AMD 37.437 / 37.672 ms, Ignition with the sigmoid epilogue 11.033 / 11.011, so **3.41x**. The ratio MOVED UP from the benchmark's 2.53x and the reason is the decode - Ignition decodes in 0.092 ms where the benchmark charged the engine arm about 4.5 ms of numpy ON BOTH ARMS; AMD gained too (5.270 -> 1.254 ms) but its frame is dominated by session.run at 34.259. The epilogue costs 0.302 ms here against the 0.260 ms the benchmark measured for it independently, so the two harnesses agree. Ignition runs the model with NO SDK CHANGE, reading reg_max from the manifest, boxes from the NPU heads on 500/500 frames, and exactly 2 onnxruntime run calls per frame for the container's two host segments (0 for YOLOv8n, 1 for YOLO11n), counted in a harness. 5.00 detections a frame with the epilogue against 6.00 without and AMD's 6: the sixth is a HardSigmoid quantization artifact the float model does not produce either. 201 MB resident against AMD's 362.6. NOT established: accuracy was not re-run (32.55 vs 23.64 is 5_eval_map.py's, and float is 39.66, so the win is against AMD on the same weights and not against float), energy was not measured, and both AMD runs reported "EP report node devices report not found" so the 12/1,526 placement is the earlier session's - [MODEL_ZOO](../../docs/MODEL_ZOO_BENCHMARKS.md#yolo26n-through-ignitions-own-pipeline-against-amds-stack-2026-09-21-desktop-2) |
| [epilogue_yolo11n_yolow_20260921.log](epilogue_yolo11n_yolow_20260921.log) | The relaxed silu_sigmoid guard applied to the two other attention models. YOLO11n: the shipped /model.10/ carve is the WHOLE C2PSA block (97 nodes, 12 rewritten) and is still correctly refused - the obstacle was the convolutions swept in with the attention, not the attention. Carving only the attention core (33 nodes, 0 overlap) permits the epilogue: 91/91 layers exact against silu_sigmoid.reference_model, and 25.80 -> 34.63 mAP on the full 5,000, with the float ceiling at 38.72, so the loss falls 12.92 -> 4.09. Cost +0.257 ms, 2.22x -> 2.20x AMD. YOLO26n priced the same lever at 8.81 mAP for 0.260 ms. HardSigmoid's 25.80 sits on AMD's recorded 25.82 for the same weights; detections 7 -> 5 on bus.jpg, the extra two being quantization artifacts. YOLO-WORLD v2 IS NOT UNLOCKED and never was blocked by host regions: silu_sites is fitted for a SiLU whose Mul output is quantized at scale 1/128 zero point 128 and YOLO-World's are not, so the epilogue is unavailable to it regardless. ALSO FIXES A DEFECT in the relaxation itself - it leaked a bare AssertionError for exactly that case where the old blanket guard gave a clean message; now a ValueError naming the precondition, with a regression test - [BENCHMARKS](../../docs/BENCHMARKS.md#the-sigmoid-epilogue-on-the-other-two-attention-models-yolo11n-gains-883-map-yolo-world-cannot-take-it-2026-09-21-desktop-2) |
| [yolo11n_carve_compare_20260921.log](yolo11n_carve_compare_20260921.log) | Closes the open question from epilogue_yolo11n_yolow_20260921.log: does the attention-core carve cost anything against the /model.10/ block carve the container ships? NO - it is better on every axis. The shipped yolo11n.ignite was NOT used as the control because it predates the band layout and would confound carve with layout, so the block carve was rebuilt today with the same compiler. ACCURACY IS UNCHANGED, and not merely to two decimals of mAP: 715,908 detections each, IDENTICAL IN ORDER, dumps the same size to the byte - which is what a layer-exact container should do when only the NPU/host boundary moves. SPEED: core 19.505 vs block 20.246 ms, 0.741 ms faster against a 0.113 ms round spread, all of it in the forward stage (13.025 -> 12.214) because the C2PSA convolutions are cheaper on the NPU than on ORT CPU. With the epilogue the core container reads 34.63 mAP at 19.788 ms - 8.83 mAP MORE accurate than what ships and still 0.458 ms FASTER. Recommendation: YOLO11n should ship the attention-core carve with the epilogue; no measured axis favours the block carve - [BENCHMARKS](../../docs/BENCHMARKS.md#yolo11ns-carve-the-narrow-one-is-free-on-accuracy-074-ms-faster-and-unlocks-883-map-2026-09-21-desktop-2) |
| [engine_linked_program_size_desktop2_20260921.log](engine_linked_program_size_desktop2_20260921.log) | The per-opcode census prices the KERNEL OBJECT; program memory holds the LINKED core. tools/engine_linked_size.py (llvm-size and llvm-objdump on eight existing builds, no compile, no device): the wrapper IRON generates is a constant 880 B (core loop 768 + _main_init 80 + __start 32) across three different engine.cc objects, all 16 cores agreeing. The committed int8 engine is 15,280 B as an object and 16,160 B linked - 224 B free, not the 1,104 B the census reading gave - so an engine.cc object may be at most 15,504 B. An undocumented experiment build links at 16,368 B, 16 B from the limit. Also the first source for the bf16 engine core: object 3,520 B, linked 4,048 B in its one-core harness (wrapper 528 B). A size, not a latency. |
| [engine_census_widened_desktop2_20260921.log](engine_census_widened_desktop2_20260921.log) | The per-opcode census with a wider reading beside the legacy one. Legacy fields reproduce the 2026-09-18 rows exactly (13,424 / 13,648 / 13,888 / 14,304 / 15,280 B, 0 accumulator moves). Widened (vst, vlda AND vldb; sp and frame-pointer aliases; am, bm and cm classes; counted again inside hardware loops alone): 0 accumulator stack moves, 31 vector stack moves, none of either inside a hardware loop - the conv pass keeps 8 live 1024-bit accumulators in cm0-cm7. Each row also states the fit against the 15,504 B object budget and every MAC loop's bundles/MACs. Static, no device. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_census_without_desktop2_20260921.log](engine_census_without_desktop2_20260921.log) | What removing a dispatch group no compiled container reaches would free (tools/engine_opcode_census.py --without, copies only, engine.cc untouched): without OP_FUSED_CONV 11,664 B (-3,616), OP_MUL 14,896 (-384), OP_SCALE 14,592 (-688), OP_POOL 14,176 (-1,104), all four 9,824 B (-5,456) - byte-identical in size to the object inside the 2026-09-17 builds. Priced, not removed. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_bf16_census_desktop2_20260921.log](engine_bf16_census_desktop2_20260921.log) | The first census of the bf16 engine core (--source kernels/bf16_conv/engine_bf16.cc): 3,520 B, 771 instructions, 0 accumulator and 0 vector stack moves under the widened reading that can see bm* accumulators; MAC loops 29/8, 24/8, 18/4, 21/4 bundles per MACs. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_loops_int8_desktop2_20260921.log](engine_loops_int8_desktop2_20260921.log) | tools/aie_disasm.py --loops --regs on the shipped int8 engine object, never run on it before. Hardware-loop bundles = cycles per iteration: stride-1 dual 23 (twice, the pass is instantiated for groups 0-1 and 2-3), stride-2 dual 32, single group 17 - and fused_stage2's loop 9 for the single-group loop's exact work, loads software-pipelined under the MACs. Why one pipelines and the other does not is unexplained. All 9 accumulator and 12 vector registers named. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_loops_bf16_desktop2_20260921.log](engine_loops_bf16_desktop2_20260921.log) | The same reading of the bf16 engine core object: stride-1 dual 24 bundles for 8 vmac.f, stride-2 dual 29, single group 18 and 21; 96 B frame. One third of a MAC issue per cycle, which is most of the distance to the 20.0 percent of ceiling measured on milestone 1. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_bank_check_int8_desktop2_20260921.log](engine_bank_check_int8_desktop2_20260921.log) | tools/aie_bank_check.py on a placed int8 engine core: weight ping-pong buffers in banks 2 and 3, activation ping-pong buffers in banks 0 and 2, so a0_0_cons_buff_1 shares bank 2 with w0_0_cons_buff_0; the stride-1 dual loops each carry 4 bundles pairing a weight load with an activation load. A HAZARD by the measured same-bank paired-load rule (predicted 23 -> 27 cycles when those two buffers meet), not a timing. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [engine_bank_check_bf16_desktop2_20260921.log](engine_bank_check_bf16_desktop2_20260921.log) | The same check on the bf16 one-core harness: activations in banks 0-1, weights in banks 2-3 (several buffers straddle a bank boundary under the sequential allocator), so its 6 weight+activation paired loads per stride-1 dual iteration do not collide. Recorded because the timing anchor stands on this build. - [BENCHMARKS](../../docs/BENCHMARKS.md#static-readings-of-both-engine-cores-a-wider-census-what-unreachable-dispatch-costs-and-a-9-cycle-loop-beside-a-17-cycle-one-2026-09-21-desktop-2) |
| [bf16_conv_npu_20260921.log](bf16_conv_npu_20260921.log) | Milestone 1: a fixed-shape bf16 convolution (kernels/bf16_conv/conv_bf16.cc) on one core, four shapes, every element bit-exact against an fp32-accumulate reference. Correctness only. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [bf16_conv_bench_npu_20260921.log](bf16_conv_bench_npu_20260921.log) | Milestone 1, accumulators in flight on identical work (147,456 MACs): 1 -> 24.44, 2 -> 31.13, 4 -> 36.64, 8 -> 45.35 GFLOPS amortised. Its CPU-relative ratios are inflated by per-call overhead on so small a tile and are not to be quoted. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [bf16_conv_bench_large_npu_20260921.log](bf16_conv_bench_large_npu_20260921.log) | Milestone 1 on an 8x32 tile, 589,824 MACs: 12.81 us per pass amortised, 92.10 GFLOPS, 20.0 percent of the 460.8 GFLOPS core ceiling, 2.76x one Zen 4 thread of torch bf16; 577.9 us for a single dispatch. timing_eligible true. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [engine_bf16_npu_20260921.log](engine_bf16_npu_20260921.log) | Milestone 2's first run: the header-parameterised engine core on five shapes, all 1,600 elements equal. Superseded as evidence of the arithmetic by engine_bf16_mac_model_probe_npu_20260921.log: it compared float values (so -0.0 equals +0.0) against a reference whose summation order was undefined, on data that cannot tell accumulate models apart. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [engine_bf16_mac_model_probe_npu_20260921.log](engine_bf16_mac_model_probe_npu_20260921.log) | The bf16 engine core under a true byte comparison, and a probe of its multiply-accumulate. Ten cases, 1,600 of 1,600 bytes equal in each, including the never-run k5, 8-block k1, 4-block k3 with ReLU6 and two-packet F_LOAD_PSUM chains inside one dispatch - and on every one of them four different accumulate models all score 1,600, so a random sweep selects nothing. The probe (77 experiments of exact powers of two, 3,200 elements) does: only the ALIGNED model - nine operands aligned to the largest exponent, each rounded separately to a 24-bit grid, ties to even - matches all 3,200; ordered fp32 1,568 and 1,595, dot-first 1,540 and 1,594, exact-sum 1,460 and 1,599. checks-only, no timing claim. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [bf16_accum_fidelity_desktop2_20260921.log](bf16_accum_fidelity_desktop2_20260921.log) | What the aligned multiply-accumulate costs where a model can see it, offline against the emulator's own mac() and an fp64 reference. Over the tap counts real layers have - to 144, k3 over 128 channels, and the post-ReLU case where errors accumulate instead of cancelling - aligned tracks an exact sum to 2.17e-07 against its 1.61e-07, and NO lane differs after the bf16 store. The grid keeps 24 bits below an instruction's largest operand and a bf16 store keeps 8, so the difference sits 16 bits under what reaches the next layer. Bracketed rather than asserted: the two agree to a cancellation ratio of 2^22 and diverge at 2^24, which is the grid's quantum and not the store's width. So the CPU bf16 score is a prediction of silicon, not only a bound on it. Synthetic Gaussian data, not MODNet-Cut's own tensors; checks-only, offline, no timing claim. Qualified 2026-09-22: "no lane" was 128 lanes a case, and on SESR-M7's own tensors the store does see the grid, rarely (bf16_engine_vs_oracle_desktop2_20260922.log). - [BENCHMARKS](../../docs/BENCHMARKS.md#the-cores-aligned-accumulate-is-invisible-after-a-bf16-store-and-the-cpu-bf16-score-survives-it-2026-09-21-desktop-2) |
| [engine_int8_loop_variants_census_desktop2_20260921.log](engine_int8_loop_variants_census_desktop2_20260921.log) | The static half of the int8 `ptr` port gate, on copies of engine.cc under scratch/ with the tracked file untouched. The hot loops go 23 to 18 bundles per 8 MACs (stride-1 dual), 32 to 27 (stride-2 dual) and 17 to 15 per 4 MACs, with 0 accumulator stack moves. The object SHRINKS, 15,280 to 15,184 B, against the +80 B the bf16 sitting measured and budgeted for - free space under the 15,504 B object budget goes 224 to 320 B. A variant that also pointer-walks fused_stage1 is byte-identical, because that loop's trip count is already constant. No latency, no device. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-makes-the-int8-engines-object-smaller-not-bigger-and-takes-a-fifth-off-its-hot-loop-2026-09-21-desktop-2) |
| [engine_int8_ptr_verify_yolov8n_npu_20260921.log](engine_int8_ptr_verify_yolov8n_npu_20260921.log) | YOLOv8n on the ptr kernel, workspace reuse OFF so all 66 tensors are readable after one dispatch: 66/66 exact, PASS. Dispatch mean 7.099 ms over 20 against the base arm's 7.172 - ranges overlap and this is NOT a timing result, only evidence the frame does not move by anything like the loop's 21.7%. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [engine_int8_base_verify_yolov8n_npu_20260921.log](engine_int8_base_verify_yolov8n_npu_20260921.log) | The committed kernel's control for the run above, same configuration, same sitting: 66/66 exact, PASS, dispatch mean 7.172 ms over 20. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [engine_int8_ptr_verify_yolov8s_npu_20260922.log](engine_int8_ptr_verify_yolov8s_npu_20260922.log) | YOLOv8s on the ptr kernel, reuse OFF: 66/66 exact, PASS. Taken because the latency win is claimed on YOLOv8s and byte-exactness had only been shown on YOLOv8n. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [ptr_ab_yolov8n_phoenix_20260922T0315Z.log](ptr_ab_yolov8n_phoenix_20260922T0315Z.log) | The interleaved A/B on YOLOv8n, AMD then control then test, twice, 500 timed frames each. G2G 10.487 AMD, 8.067 control, 7.841 ptr; every control round slower than every test round; -2.8% and 1.30x to 1.34x against AMD. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [ptr_ab_yolov8s_phoenix_20260922T0317Z.log](ptr_ab_yolov8s_phoenix_20260922T0317Z.log) | The same on YOLOv8s, and the one that changes a standing claim. G2G 16.949 AMD, 17.627 control, 16.855 ptr: the committed kernel loses to AMD by 0.678 ms and ptr wins by 0.094, with ptr's slower round below AMD's faster one. One sitting, one image, 0.55% margin. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [ptr_ab_yolov8s_s2_phoenix_20260922T0325Z.log](ptr_ab_yolov8s_s2_phoenix_20260922T0325Z.log) | The second YOLOv8s sitting, taken because the first one's 0.094 ms margin over AMD was too thin to stand alone, and wrapped so it carries the platform manifest and timing_eligible: true that the first pair lacks (wrapper: [engine_int8_ptr_ab_yolov8s_sitting2_npu_20260922.log](engine_int8_ptr_ab_yolov8s_sitting2_npu_20260922.log)). G2G 17.135 AMD, 17.415 control, 16.665 ptr. The control-to-test delta reproduces at -4.3% against -4.4%; the margin over AMD does not, because AMD drifted 16.949 to 17.135. Across both sittings ptr is faster than AMD in all eight arms, 16.883 worst against 16.916 best. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) |
| [ptr_ab_yolov8x_split_phoenix_20260922T0340Z.log](ptr_ab_yolov8x_split_phoenix_20260922T0340Z.log) | The final ptr test and the one that exposes a scope limit. YOLOv8x, 106 layers, 145.49 MB of weight packets, as a three-segment split container cut at layers 35 and 71 - so this is also the only ptr A/B on the multi-segment dispatch path. G2G 78.684 AMD, 123.353 control, 115.936 ptr: ptr's largest margin at -7.417 ms and -6.01%, ranges disjoint, completing the trend -2.8% (n), -4.4% (s), -6.0% (x). AND Ignition LOSES this model to AMD by 1.47x, which ptr narrows and nowhere near closes, so the engine's standing against AMD inverts across the family and YOLOv8s is its crossover point rather than a narrow win. Wrapped, timing_eligible true (wrapper: [engine_int8_ptr_ab_yolov8x_split_npu_20260922.log](engine_int8_ptr_ab_yolov8x_split_npu_20260922.log)). - [BENCHMARKS](../../docs/BENCHMARKS.md#yolov8x-ptr-scales-to--60-and-the-engines-standing-against-amd-inverts-with-model-size-2026-09-22-desktop-2) |
| [engine_int8_ptr_verify_yolov8x_split_npu_20260922.log](engine_int8_ptr_verify_yolov8x_split_npu_20260922.log) | The ptr kernel's widest correctness check: YOLOv8x as a three-segment split container, workspace reuse off so all 106 tensors are readable after one dispatch, 106/106 exact, PASS. This is the only byte-exactness run in the ptr set that crosses segment boundaries, where workspace state carries between separate instruction streams. - [BENCHMARKS](../../docs/BENCHMARKS.md#yolov8x-ptr-scales-to--60-and-the-engines-standing-against-amd-inverts-with-model-size-2026-09-22-desktop-2) |
| [engine_dispatch_cost_model_desktop2_20260922.log](engine_dispatch_cost_model_desktop2_20260922.log) | DERIVED, no device: what a whole frame is bound by, composed from constants already measured here and counts read out of the ptr-gate containers. Dispatch is linear in INSTRUCTION-STREAM bytes at 16.01 / 16.08 / 15.09 ns per byte on YOLOv8n/s/x - a 1.066x spread across a 16.5x range of frame time - which at 145 ns per op is 9.06 / 9.02 / 9.61 bytes per op and confirms the op cost from whole frames rather than from appending BD writes to one. An activation-packet model with no fitted parameter explains 75 / 92 / 94% and is downstream of it. The chain: activation re-fetch 5.6x to 31.6x (a LOWER BOUND, against peak workspace not tensor bytes), packets 21.6x, instruction bytes 18.1x, frame 16.5x. Explains why the MemTile activation ring lost - it cut bytes, which are not the cost, and added protocol ops, which are. Derived, not measured; does not separate core stall from shim critical path. - [BENCHMARKS](../../docs/BENCHMARKS.md#what-a-whole-frame-is-bound-by-the-instruction-stream-not-the-bytes-and-not-the-loop-derived-2026-09-22) |
| [engine_bf16_bench_npu_20260921.log](engine_bf16_bench_npu_20260921.log) | The first timing of the bf16 engine core, against milestone 1 on identical work (3x3, 32 -> 16 channels, 5x20 tile, 460,800 MACs), alternating five rounds in one process, 128 in-core repeats: engine 10.02-10.48 us per pass dispatch-free, median 10.098, 91.27 GFLOPS, 19.8 percent of ceiling; fixed-shape kernel 8.85-9.02, median 8.956, 102.9 GFLOPS. 12.8 percent slower, ranges disjoint, difference unattributed. timing_eligible true, host CLEAR. - [BENCHMARKS](../../docs/BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_census_desktop2_20260921.log](engine_bf16_loop_variants_census_desktop2_20260921.log) | Static census of nine arithmetic-neutral rewrites of the bf16 engine's hot loop, each a copy under scratch (tools/engine_bf16_loop_variants.py): base 24 bundles per 8 MACs; running pointers 19; a flattened tap table 32 (worse); constant-trip loops of 4/2/1 over the input blocks 16 (the same density as fused_stage2's loop - a trip count the compiler can see is the trigger) at +2,672 B; hoisted bias no change. 0 accumulator and 0 vector stack moves in every one. - [BENCHMARKS](../../docs/BENCHMARKS.md#five-arithmetic-neutral-rewrites-of-the-bf16-hot-loop-cut-its-bundles-by-up-to-a-third-and-the-wall-clock-did-not-move-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_exact_npu_20260921.log](engine_bf16_loop_variants_exact_npu_20260921.log) | The chunk variant (the largest rewrite) on the device: byte-exact on all ten sweep cases including the two-packet F_LOAD_PSUM chains, and its multiply-accumulate probe answers identical to the committed kernel's (aligned 1,600/1,600 on both sets). checks-only, no timing claim. SUPERSEDED the same day: this run executed the ptr variant's cached design, not chunk's - the source was not in the jit cache key (engine_bf16_cache_audit_desktop2_20260921.log); chunk's own run is engine_bf16_loop_variants_exact_chunk_npu_20260921.log. - [BENCHMARKS](../../docs/BENCHMARKS.md#five-arithmetic-neutral-rewrites-of-the-bf16-hot-loop-cut-its-bundles-by-up-to-a-third-and-the-wall-clock-did-not-move-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_bench_k3_npu_20260921.log](engine_bf16_loop_variants_bench_k3_npu_20260921.log) | Timing, k3 over 4 input blocks, five alternating rounds, 128 in-core repeats, timing_eligible true: base 10.093 us per pass dispatch-free; ptr 10.168; chunk 10.228; hoist 10.094; hoist+ptr 10.134; milestone-1 fixed-shape 9.173. Static loop savings of 21-33 percent produced -0.0 to +1.3 percent: the loop's bundle count is not its cycle count on this engine, as H11 found on the GEMM. SUPERSEDED the same day: every engine arm here ran the same two cached designs (single-dispatch ptr, amortised the committed kernel), so the arms could not differ; the rerun with the objects named is engine_bf16_loop_variants_bench_k3_npu_20260921_02.log. - [BENCHMARKS](../../docs/BENCHMARKS.md#five-arithmetic-neutral-rewrites-of-the-bf16-hot-loop-cut-its-bundles-by-up-to-a-third-and-the-wall-clock-did-not-move-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_bench_k1_npu_20260921.log](engine_bf16_loop_variants_bench_k1_npu_20260921.log) | The same arms at k1 over 8 input blocks: base 3.198 us, ptr 3.204, chunk 3.209, hoist 3.239, hoist+ptr 3.239, milestone-1 2.084. The base loop bodies are 1.47 us of the 3.20 (DERIVED), so more than half of a 1x1 pass is per-pass cost outside the loop, and hoisting the bias is not where it goes. Unattributed. SUPERSEDED the same day, same cause as the k3 row; the rerun is engine_bf16_loop_variants_bench_k1_npu_20260921_02.log, where hoisting the bias IS 9.7 percent of a k1 pass. - [BENCHMARKS](../../docs/BENCHMARKS.md#five-arithmetic-neutral-rewrites-of-the-bf16-hot-loop-cut-its-bundles-by-up-to-a-third-and-the-wall-clock-did-not-move-2026-09-21-desktop-2) |
| [engine_bf16_cache_audit_desktop2_20260921.log](engine_bf16_cache_audit_desktop2_20260921.log) | Static, the jit cache read before any rerun (tools/engine_bf16_cache_audit.py): six bf16 engine designs, each named by the digest of the source it compiled and the .text size of the object it linked. No chunk, hoist or hoist+ptr entry exists - the sweeps and timing arms that reported them ran the ptr (repeat 1) and committed-kernel (repeat 128) designs, because the source was chosen inside the jit generator and so was not in the cache key. The evidence behind the supersession of the three rows above. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_cache_audit_desktop2_20260921_02.log](engine_bf16_cache_audit_desktop2_20260921_02.log) | The same audit after the reruns: twenty designs, every variant present at repeat 1, 2 and 128 with its own object (chunk 6,192 B, hoist 3,456, hoist+ptr 3,552, ptr 3,600, committed 3,520). - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_exact_chunk_npu_20260921.log](engine_bf16_loop_variants_exact_chunk_npu_20260921.log) | The chunk variant on the device with its object named in the log (ENGINE_BF16_DESIGN: 6,192 B, two fresh cache entries): byte-exact on all ten sweep cases including the two-packet F_LOAD_PSUM chains, accumulate probe aligned 1,600/1,600 on both sets. checks-only, no timing claim. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_exact_hoist_npu_20260921.log](engine_bf16_loop_variants_exact_hoist_npu_20260921.log) | The hoist variant (3,456 B, never swept before): byte-exact on all ten cases, probe aligned 1,600/1,600 on both sets. checks-only. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_exact_hoist_ptr_npu_20260921.log](engine_bf16_loop_variants_exact_hoist_ptr_npu_20260921.log) | The hoist+ptr variant (3,552 B): byte-exact on all ten cases, probe aligned 1,600/1,600 on both sets. checks-only. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_exact_ptr_npu_20260921.log](engine_bf16_loop_variants_exact_ptr_npu_20260921.log) | The ptr variant (3,600 B) again, this time with its object in a tracked log: byte-exact on all ten cases, probe aligned 1,600/1,600 on both sets. checks-only. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_bench_k3_npu_20260921_02.log](engine_bf16_loop_variants_bench_k3_npu_20260921_02.log) | Timing with the objects named, k3 over 4 input blocks, five alternating rounds, 128 in-core repeats, ten distinct engine designs alive in one process, timing_eligible true, host CLEAR: committed kernel 10.277 us per pass dispatch-free; ptr 9.238 (-10.1 percent); chunk 8.460 (-17.7 percent, 108.9 GFLOPS, below every other arm's five rounds); hoist 10.093 (inside the spread); hoist+ptr 9.173; milestone-1 fixed-shape 9.165. The bundle-count prediction was -14 and -22 percent: the static reading was right. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bf16_loop_variants_bench_k1_npu_20260921_02.log](engine_bf16_loop_variants_bench_k1_npu_20260921_02.log) | The same arms at k1 over 8 input blocks: committed 3.310 us, ptr 3.094 (-6.5 percent), chunk 2.858 (-13.7), hoist 2.988 (-9.7), hoist+ptr 2.969 (-10.3, one round at 4.03), milestone-1 2.131. Hoisting the bias out of the pass is worth 0.32 us at k1. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2) |
| [engine_bank_placement_probe_desktop2_20260921.log](engine_bank_placement_probe_desktop2_20260921.log) | Compile-only, five xclbins built and none run (tools/engine_bank_placement_probe.py, design.py untouched): the int8 engine's a*_cons_buff_1 shares a bank with a weight buffer under bank-aware (reproduces every shipped build), under basic-sequential (with three buffers straddling banks), and with psum pinned to bank 3 or 2; pinning scratch as well, to force the free bank, makes the allocator FAIL rather than use it. A collision-free placement exists on paper and needs explicit addresses on ObjectFifo buffers, which IRON does not expose. A placement result, no latency. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-int8-engines-weightactivation-bank-collision-cannot-be-placed-away-with-the-levers-iron-exposes-2026-09-21-desktop-2) |
| [engine_dispatch_gate_census_desktop2_20260922.log](engine_dispatch_gate_census_desktop2_20260922.log) | Static, no device (tools/engine_dispatch_gate_census.py): compiling out the four dispatch groups no container reaches frees 5,408 B, 15,184 to 9,776 B, taking headroom from 320 to 5,728 B of the 15,504 B object budget. Per group FUSED_CONV 3,568, POOL 960, SCALE 560, MUL 320 - exactly additive. The -D gate and the text strip the 2026-09-21 census priced produce byte-identical objects (PASS), and the guards are inert against the pre-gate kernel (PASS). An object size, not a latency. - [BENCHMARKS](../../docs/BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2) |
| [engine_dispatch_gate_yolov8n_desktop2_20260922.log](engine_dispatch_gate_yolov8n_desktop2_20260922.log) | Silicon, checks-only: a gated YOLOv8n container (kernel object 9,776 B, kernel_ops CONV+MAXPOOL+RESIDUAL) is 66/66 layers byte-exact against the integer reference on a --no-workspace-reuse build; device released with no contexts. Its weight packets and instruction stream are byte-identical to the ungated control's, so the gate changes only the core program. - [BENCHMARKS](../../docs/BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2) |
| [engine_dispatch_gate_census_desktop2_20260922_02.log](engine_dispatch_gate_census_desktop2_20260922_02.log) | The census above re-run with the loop, container and survey readings the first lacked, and it changes the conclusion: five production loops survive untouched, five are removed with the gated-out cases, and ONE stride-2 dual loop goes 27 to 28 bundles - so the gate is not a pure program-memory change and owes the A/B below. Also: gated and ungated containers carry byte-identical wpackets.bin and insts.bin (xclbin differs by 87,056 B), and all 12 containers on this machine derive one opcode set, CONV+MAXPOOL+RESIDUAL. Supersedes the first census run. - [BENCHMARKS](../../docs/BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2) |
| [gate_ab_yolov8n_phoenix_20260922T0455Z.log](gate_ab_yolov8n_phoenix_20260922T0455Z.log) | Same-sitting interleaved A/B, gated against ungated with AMD as the drift reference, 50 warm-up + 500 timed frames, two rounds: the gated build costs +0.030 ms of dispatch (6.969/6.960 to 7.000/6.990), +0.43 percent, ranges disjoint across both rounds, and +0.31 percent glass-to-glass. The price of 5,408 B of program memory, attributable to the one loop that gained a bundle. - [BENCHMARKS](../../docs/BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2) |
| [engine_bf16_contract_npu_20260922.log](engine_bf16_contract_npu_20260922.log) | Silicon, checks-only: the bf16 core rewritten to meet its emulator's contract (a switch with a default, the 4-to-8 interleave_zip store, a sixth scratch argument) is byte-exact on ten cases, 1,600 of 1,600 bytes each, nan 0. Object .text 3,520 to 7,216 B of a 15,504 B budget, 0 accumulator stack moves. OP_RESIDUAL and F_HOLD are implemented here but NOT exercised - the sweep sends convolutions only. Superseded as coverage by the 13-case run below. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-core-meets-its-emulators-contract-and-op_residual-and-f_hold-reach-silicon-2026-09-22-desktop-2) |
| [engine_bf16_contract_npu_20260922_02.log](engine_bf16_contract_npu_20260922_02.log) | The same kernel with three residual cases added, so OP_RESIDUAL and F_HOLD execute on silicon for the first time: 13 of 13 cases byte-exact, 1,600 of 1,600 bytes, bytes_equal_frac 1.0, nan 0. residual_chain holds its own result for a second residual - three packets in one dispatch, reading and writing scratch at the same offset. Residual fixtures carry zero weights, so a core falling through to a convolution would emit the bias, not plausible noise. Supersedes the 10-case run above. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-core-meets-its-emulators-contract-and-op_residual-and-f_hold-reach-silicon-2026-09-22-desktop-2) |
| [engine_bf16_design_l1_desktop2_20260922.log](engine_bf16_design_l1_desktop2_20260922.log) | Compile only, one xclbin built and never run (tools/engine_bf16_build.py): the first sixteen-core bf16 design that has ever been built places on all 16 cores at extent 65,024 of 65,536 - buffers 60,544 B, stack 2,048, holes 2,432, 512 B free - so the predicted 256 B overrun does not occur and w_depth stays 2. bf16 carries 3,200 B more buffer than int8 and loses 3,200 B fewer to holes, and both end at the same address. WEIGHT_ACTIVATION_SHARED_BANKS none, where int8 collides in bank 2. The linked engine_bf16.o is the same sha256 as the single-core object that passed the 13-case sweep. No dispatch is emitted: insts.bin is 16 B and this xclbin cannot be run. A placement, no latency. - [BENCHMARKS](../../docs/BENCHMARKS.md#sixteen-cores-of-bf16-place-inside-65536-b-and-the-extra-bytes-come-out-of-int8s-holes-2026-09-22-desktop-2) |
| [engine_bf16_16core_npu_20260922.log](engine_bf16_16core_npu_20260922.log) | Silicon, checks-only: the first sixteen-core bf16 dispatch there has ever been, byte-exact on every column and every iteration - 4 columns x 3 iterations at 76,800 of 76,800 bytes, 921,600 bytes compared, all equal. Seven synthetic scenarios over one 25,664 B instruction stream: k3s1 ReLU (two tiles), a 1x1 over eight blocks filling the activation packet, an accumulate/emit chunk pair that runs BOTH loops of _core_fn and carries psum between weight packets, a hold/residual pair that crosses the scratch aliasing in the accumulate-only loop, and a stride-2 downsample. Establishes on hardware what the placement build could not: element-unit split/join offsets, count_out and count_acc from header words 5 and 6, the MemTile join of four bf16 tiles, and sixteen cores in lockstep. Needed no schedule - the synthetic path never imports engine_schedule. Kernel object sha256 67d1050f, the same as the single-core sweep. timing_eligible false; the milliseconds are wall clock around a correctness run and are not a latency. - [BENCHMARKS](../../docs/BENCHMARKS.md#sixteen-cores-run-bf16-byte-exactly-on-silicon-2026-09-22-desktop-2) |
| [bf16_integer_exactness_20260922.log](bf16_integer_exactness_20260922.log) | Offline, checks-only, no device (tools/bf16_integer_exactness.py): integer operands through the MEASURED aligned multiply-accumulate are bit-exact against an int64 reference while the running sum stays under 2^24 = 16,777,216, and the bound is the accumulator grid rather than the operand width. uint8 x int8 and int8 x int8 random data are exact at every depth to 18,432 MACs, peaking at 5,035,709 - a 3.3x margin. The adversarial arm, every product 255 x 127 with the same sign, is exact to 9,326,880 at 2,304 MACs and breaks at 4,608 MACs with deviation 56.0 and a peak of 18,653,760. uint8 and int8 both round-trip bf16 losslessly, asserted per tap, so this tests the adder and not the cast. NOT a working int8 path: the output store is bf16, so an accumulator above 256 needs an epilogue scale that epilogue() does not have. Emulator mac() in aligned mode, which is the measured model of the core, not the core. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-datapath-carries-integer-work-exactly-and-the-bound-is-224-2026-09-22-offline) |
| [engine_bf16_16core_integer_npu_20260922.log](engine_bf16_16core_integer_npu_20260922.log) | Silicon, checks-only (engine_bf16_16core.py --integer): the bf16 datapath carrying INTEGER operands on all sixteen cores, 4 columns x 3 iterations at 76,800 of 76,800 bytes equal every time, 921,600 bytes compared, all exact, over the same seven scenarios as the float sweep. The compile census first proves it is an integer plan and not a rerun with different numbers: 153,600 expected values, 0 non-integer, max abs 69.0, store_lossless true - and the harness REFUSES to build a plan whose expected outputs are not integers, so the run cannot become a test of the bf16 store. k1_chunk1 loads psum back across a weight-packet boundary, so float32 psum carrying integers exactly between packets is confirmed on hardware. Operands are act [0,3], weight [-1,1], bias [-4,4] - small on purpose, because 72 MACs x 3 x 1 = 216 keeps every accumulator under the 256 the bf16 store carries. CONFIRMS the datapath, NOT the 2^24 bound, which stays an emulator result until the epilogue has an output scale. Float arm inert: a default compile still gives insts 25,664 and kernel object 67d1050f. timing_eligible false; the milliseconds are wall clock around a correctness run. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-bf16-datapath-carries-integer-work-exactly-and-the-bound-is-224-2026-09-22-offline) |
| [bf16_packet_recount_20260922.log](bf16_packet_recount_20260922.log) | Offline, checks-only, no device (tools/bf16_packet_recount.py): per-layer activation packet recount under bf16 TILE geometry, correcting the dispatch estimator's hardcoded 'activation packets x1'. A bf16 output tile carries 16 channels where an int8 tile carries 32, so a layer needs ceil(Cout/16) groups against ceil(Cout/32) and every group re-reads the input plane. SESR-M7 ratio 1.0000 - every layer is 16 output channels or fewer (head and seven body convs Cout 16, tail 12), so one group serves it at either width and int8 is wasting half its output tile. MODNet-Cut ratio 1.8841 - 74,276 to 139,944 activation packets, rounds 4,992 to 8,976. Counts and ratios only, no latency. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-activation-packet-count-does-not-stay-fixed-and-that-made-the-bracket-too-kind) |
| [bf16_dispatch_estimate_corrected_20260922.log](bf16_dispatch_estimate_corrected_20260922.log) | Offline, checks-only (tools/bf16_dispatch_estimate.py with --activation-packet-ratio now REQUIRED): MODNet-Cut's bf16 bracket re-run at ratio 1.884108. Frame 163.02 ms low to 217.28 ms high, 5.24x to 6.98x AMD's measured 31.117 ms - the corrected FLOOR is above the superseded ceiling of 168.98 ms. Dispatch 77.19-78.49 ms, transfer 59.89-112.84 ms, host 22.50 ms. The bracket is narrower than before because the activation count is now counted rather than guessed; what remains uncertain is only the weight side. DERIVED, not measured. Supersedes the 2026-09-22 bracket of 128.84-168.98 ms. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-activation-packet-count-does-not-stay-fixed-and-that-made-the-bracket-too-kind) |
| [bf16_engine_vs_oracle_desktop2_20260922.log](bf16_engine_vs_oracle_desktop2_20260922.log) | Offline, checks-only, no device (tools/bf16_engine_vs_oracle.py, aligned MAC model): design B's W8A16 numerics on SESR-M7, the bf16 engine's emulator chained over all nine layers against tools/w8a16_oracle.py built from sesr_m7_xint8.onnx (the plain XINT8 arm). Five Set5 images plus seed 0. At the output 0 to 58 of 786,432 values differ, the largest difference is at most one bf16 step at the tensor's peak on every layer of every input, output rel_l2 at most 1.59e-5 against 1.63e-3 to 1.92e-3 for the bf16 step itself; the worst layer is 151 of 1,048,576 at body.4 on head.png. The fork's schedule, emulated on a NaN background, differs from the chained reference in 0 patterns on every layer. Isolated layers against a float64 sum: the engine leaves it at 80 values, the oracle at 30. The tolerance the silicon run is held to, not a silicon result. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-on-the-bf16-engine-sesr-m7-stays-within-one-bf16-step-of-its-oracle-and-the-cores-aligned-accumulate-is-not-the-gap-2026-09-22-offline) |
| [bf16_engine_vs_oracle_wide_desktop2_20260922.log](bf16_engine_vs_oracle_wide_desktop2_20260922.log) | The control for the row above: the same six inputs with the "wide" MAC model, which sums each instruction's nine operands exactly, schedule not emulated. The engine still leaves the float64 sum at 72 values against aligned's 80, the chained worst figures do not move (151 values, rel_l2 1.18e-4, 1.0 step) and head.png is identical layer for layer - so the core's aligned grid is about 8 of the 80 and is not what separates engine from oracle. Why ORT's fp32 order keeps the exact value more often on real images is unexplained. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-on-the-bf16-engine-sesr-m7-stays-within-one-bf16-step-of-its-oracle-and-the-cores-aligned-accumulate-is-not-the-gap-2026-09-22-offline) |
| [sesr_m7_bf16_compile_desktop2_20260922.log](sesr_m7_bf16_compile_desktop2_20260922.log) | Compile only, checks-only, no device: the first bf16 container, `ignite-compile --engine graph --datapath bf16` on sesr_m7_xint8.onnx through compiler/engine_schedule_bf16.py. build/sesr_m7_bf16.ignite, 495,744 B in 7.0 s: xclbin 162,942 B, instruction stream 144,880 B, weight packets 170,496 B, 1,521 rounds, workspace about 11.7 MB, egress 1,572,864 B (12 x 256 x 256 at two bytes). Rounds and stream length equal the int8 container's in bf16_packet_recount_20260922.log, and the packets are the same 18 of 9,472 B, as the recount predicted for a model of 16 channels or fewer. No session can run it yet: every one refuses it at admission. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-first-bf16-container-compiles-and-nothing-can-run-it-yet) |
| [sesr_m7_int8_rebuild_desktop2_20260922.log](sesr_m7_int8_rebuild_desktop2_20260922.log) | Compile only, checks-only (tools/rebuild_container_check.py): build/sesr_m7.ignite, the int8 arm of the W8A16 sitting, reproduces under the day's compiler on every compared quantity (insts 144,880 B, 7,436 activation packets, 170,496 B of weight packets, 1,521 rounds, egress 786,432 B) in 7.4 s, before it was run. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-silicon-resident-tensors-exact-to-the-emulator-041-db-over-int8-and-the-host-egress-loses-the-tile-2026-09-22-desktop-2) |
| [sesr_m7_bf16_rebuild_desktop2_20260922.log](sesr_m7_bf16_rebuild_desktop2_20260922.log) | Compile only, checks-only: the same for build/sesr_m7_bf16.ignite (egress 1,572,864 B, a_bytes 12,800), reproduced in 7.0 s before it was run. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-silicon-resident-tensors-exact-to-the-emulator-041-db-over-int8-and-the-host-egress-loses-the-tile-2026-09-22-desktop-2) |
| [sesr_m7_bf16_silicon_smoke_desktop2_20260922.log](sesr_m7_bf16_silicon_smoke_desktop2_20260922.log) | Silicon, checks-only, device idle either side: the first dispatch ever made of the bf16 SESR-M7 schedule (tools/bf16_sr_silicon_check.py --no-timing --no-quality), baby then seed 0 then baby again. Device equals emulator to the bit on both inputs (1,048,576 tail values and 5,177,344 resident values each, 0 differ); the repeat is identical; against the W8A16 oracle it reproduces the offline figures (baby 2 of 786,432). - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-silicon-resident-tensors-exact-to-the-emulator-041-db-over-int8-and-the-host-egress-loses-the-tile-2026-09-22-desktop-2) |
| [sesr_m7_bf16_silicon_desktop2_20260922.log](sesr_m7_bf16_silicon_desktop2_20260922.log) | Silicon, timing eligible, no foreign contention, power mode default: W8A16 SESR-M7 on the bf16 engine beside the int8 container, one sitting. EXACT: device equals emulator on six inputs (6,291,456 tail values, all 16 lanes, and 31,064,064 resident values, 0 differ), input region intact, a later repeat identical; against the oracle, worst 58 of 786,432 and at most one bf16 step at peak, as offline. QUALITY (5_eval tiling): W8A16 34.65 / 29.60 dB PSNR-Y on Set5 / Set14 against W8A8 34.25 / 29.41 and FP32 35.64 / 30.03, all 19 images improved; int8 container equals the QDQ model on the CPU at 0 of 11,366,412 pixels. TIMING per 256 x 256 tile, two alternating rounds: dispatch bf16 5.21-5.28 ms against int8 4.07-4.10; tile bf16 10.68-10.79 ms (numpy egress 4.72-4.87) against int8 4.55-4.59 and FP32 CPU 9.85 (run 7.47, ORT 1.30). - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-silicon-resident-tensors-exact-to-the-emulator-041-db-over-int8-and-the-host-egress-loses-the-tile-2026-09-22-desktop-2) |
| [sesr_m7_bf16_native_host_desktop2_20260922.log](sesr_m7_bf16_native_host_desktop2_20260922.log) | Silicon, timing eligible, no foreign contention, power mode default, 23:52-23:56 on 2026-09-22: the W8A16 SESR-M7 sitting again with native bf16 host paths (preprocess_simd: bgr_to_c8_plane_bf16, depth_to_space_crd_bgr_bf16) and the numpy path as a third arm. Host ends exact on device data: native egress equals numpy on 7 of 7 frames and 31 of 31 quality tiles, staged values are npu/sesr.py's preprocess on 6 of 6 images. Device equals emulator again (6,291,456 tail and 31,064,064 resident values, 0 differ); quality identical to the earlier sitting to every digit (W8A16 34.65 / 29.60 dB). TIMING per 256 x 256 tile, two alternating rounds: bf16 native host 6.28-6.29 ms (dispatch 5.24-5.27, egress 0.86-0.88, staging 0.11-0.12) against bf16 numpy host 10.65-10.79, int8 4.50-4.58 and FP32 CPU 10.75 (run 8.17, drifted from 7.47). - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-with-native-host-paths-the-tile-beats-the-cpu-and-the-frame-is-now-dispatch-bound-2026-09-22-desktop-2) |
| [sesr_m7_adaround_int8_compile_desktop2_20260923.log](sesr_m7_adaround_int8_compile_desktop2_20260923.log) | Compile only, checks-only, no device: the int8 SESR-M7 container built from sesr_m7_xint8_adaround.onnx (the plain model with 5,447 of 22,128 weight codes moved by one), 1,521 rounds, dense uint8 output. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_compile_desktop2_20260923.log](sesr_m7_adaround_bf16_compile_desktop2_20260923.log) | Compile only, checks-only, no device: the bf16 (W8A16) container from the same AdaRound model, 1,521 rounds, dense bf16 output in real units. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_vs_plain_int8_containers_desktop2_20260923.log](sesr_m7_adaround_vs_plain_int8_containers_desktop2_20260923.log) | Offline (tools/container_diff.py --rebuild-of-a): the AdaRound int8 container carries the plain one's schedule on all 26 compared quantities; kernel object and insts.bin identical; the embedded xclbin differs only inside a rebuild's own noise; only the weight packets differ. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_vs_plain_bf16_containers_desktop2_20260923.log](sesr_m7_adaround_vs_plain_bf16_containers_desktop2_20260923.log) | The same comparison for the bf16 pair: same schedule on 26 quantities, insts.bin identical, xclbin within rebuild noise. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_silicon_desktop2_20260923.log](sesr_m7_adaround_bf16_silicon_desktop2_20260923.log) | Silicon, timing eligible, no foreign contention, power mode default: W8A16 on the AdaRound weights beside its W8A8 twin, one sitting; exits 1. EXACT FAILS on 4 of 6 inputs (129 of 31,064,064 resident values, 11 of 6,291,456 tail values; head exact on every input; repeat identical; input intact). QUALITY (5_eval tiling): W8A16 35.37 / 29.93 dB PSNR-Y on Set5 / Set14, W8A8 35.06 / 29.79 (equals the QDQ on the CPU at 0 of 11,366,412 pixels), FP32 35.64 / 30.03; bf16 against the oracle 175 pixels by one level. TIMING per tile: bf16 native host 6.16-6.18 ms (dispatch 5.19-5.20) against int8 4.51 and FP32 CPU 9.95. AMD's arm not run. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_dump_desktop2_20260923.log](sesr_m7_adaround_bf16_dump_desktop2_20260923.log) | Silicon, checks-only, device idle either side: the same seven frames again with --dump-frames and --isolate all. Every count of the sitting above reproduces. Every layer whose inputs were still resident at frame end (head, body.5, body.6 with its Add, tail), replayed from the device's own inputs, is exact under the aligned model: 23,592,960 values, 0 differ. The dump itself is git-ignored; its file hashes are in the log. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_dump_replay_desktop2_20260923.log](sesr_m7_adaround_bf16_dump_replay_desktop2_20260923.log) | Offline (--from-dump), no device: the dump above checked again; every count reproduces, and each difference's position is listed: one spatial cluster per inexact frame, first visible in body.4. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [bf16_engine_vs_oracle_adaround_desktop2_20260923.log](bf16_engine_vs_oracle_adaround_desktop2_20260923.log) | Offline, checks-only, no device: tools/bf16_engine_vs_oracle.py on the AdaRound QDQ and the six inputs the plain weights used. The schedule emulation equals the direct reference on all 54 layers-by-inputs; the emulator's distance to the W8A16 oracle is worst at bird, 118 of 786,432. - [BENCHMARKS](../../docs/BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_noreuse_compile_desktop2_20260923.log](sesr_m7_adaround_bf16_noreuse_compile_desktop2_20260923.log) | Compile only, checks-only, no device: the AdaRound bf16 container with --no-workspace-reuse, so every tensor stays resident. The same 1,521 rounds and weight packets as the reuse container; a different instruction stream and a 20.3 MB workspace against 11.7. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-adaround-mismatch-starts-in-body1-as-single-values-beside-bf16-rounding-ties-and-none-of-the-four-candidate-accumulate-models-reproduces-it-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_noreuse_dump_desktop2_20260923.log](sesr_m7_adaround_bf16_noreuse_dump_desktop2_20260923.log) | Silicon, checks-only, device idle either side, no foreign context: the no-reuse container on the same seven frames with --isolate all --dump-frames; exits 1. Every tensor both containers keep has the same differing count and bounding box as with reuse, so the mismatch does not follow addresses. head and body.0 are exact; it starts in body.1 and reaches all six inputs (bird and woman carry differences that round away before body.4). Replayed layer by layer from the device's own inputs over 55,050,240 values, aligned gets 9 wrong, dot_first 14, wide 16, sequential 17; the nine are single values one bf16 step off, next to rounding ties. The log's BF16_SR_RESULT field exact_inputs counts the inputs compared, not the inputs found exact: it reads 6 beside device_equals_emulator false. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-adaround-mismatch-starts-in-body1-as-single-values-beside-bf16-rounding-ties-and-none-of-the-four-candidate-accumulate-models-reproduces-it-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_noreuse_replay_exp_sum_desktop2_20260923.log](sesr_m7_adaround_bf16_noreuse_replay_exp_sum_desktop2_20260923.log) | Offline (--from-dump --isolate all), no device: the no-reuse dump replayed layer by layer under five accumulate models, the fifth being the new candidate exp_sum. Over 55,050,240 values exp_sum gets 0 wrong; aligned 9, dot_first 14, wide 16 and sequential 17 repeat the sitting's counts layer by layer. exp_sum and aligned therefore disagree at exactly the nine positions exp_sum was fitted to, so this checks that exp_sum breaks nothing else, not that it is right. Exits 1 and reads device_equals_emulator false because aligned was the model in force. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [engine_bf16_mac_model_probe_rescore_exp_sum_desktop2_20260923.log](engine_bf16_mac_model_probe_rescore_exp_sum_desktop2_20260923.log) | Offline (engine_bf16.py --probe-log), no device: every emulator model scored against the silicon engine_bf16_mac_model_probe_npu_20260921.log recorded. exp_sum equals it at all 616 recorded elements; all five models agree on the other 2,584, so its 3,200 of 3,200 is DERIVED. The same derivation reproduces the log's own counts for the four models it scored. exp_sum and aligned predict identical bits at every element, so this probe cannot separate them. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_position_probe_predictions_desktop2_20260923.log](bf16_mac_position_probe_predictions_desktop2_20260923.log) | Offline, no device: the pre-registration of the one-core probe on the nine aligned misses. It lists the plan (66 packets: arm A 10 network tiles, arm B 48 for the nine chains, arm C 8 for twelve fresh vectors), every operand, and every model's prediction at each of 132 readouts. The two-pass readout self-check passes under all five models. Committed with the tool before any sitting; the sitting has not run. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_position_probe_npu_desktop2_20260923.log](bf16_mac_position_probe_npu_desktop2_20260923.log) | Silicon, checks-only, device idle either side, no foreign context: the pre-registered one-core probe, run. Its 165 plan, operand and prediction lines are byte-identical to the committed predictions log. All 132 readouts decode exactly and repeat across passes; the two pass-2 dispatches are identical. A: one core equals the network on all 10 tiles. B: exp_sum 108 of 108 readouts, aligned 9 (the assembled starts only). C: exp_sum 24 of 24, aligned 16. A1 + B1 + C1 for exp_sum, so the pre-registered rule is met. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-one-core-probe-measures-exp_sum-a1-b1-and-c1-on-the-pre-registered-plan-2026-09-23-desktop-2) |
| [bf16_mac_rescore_exp_sum_where_sesr_plain_desktop2_20260923.log](bf16_mac_rescore_exp_sum_where_sesr_plain_desktop2_20260923.log) | Offline, no device: tools/bf16_mac_model_rescore.py where, the pre-registration of the plain-weights --no-workspace-reuse sitting. It chains the network under every model on the sitting's six frames (five Set5 images, seed 0) and lists every value of every tensor where a model's bits part from aligned's. exp_sum parts at exactly 3 (bird body.1 c12 at y70 x179 and y121 x252, woman body.3 c13 at y186 x143), none in a padding lane and none carried downstream. wide parts at 45, dot_first at 14 and sequential at 257, which agrees with the sesr rescore's compared plus not-compared counts. Committed before the sitting. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [sesr_m7_bf16_noreuse_compile_desktop2_20260923.log](sesr_m7_bf16_noreuse_compile_desktop2_20260923.log) | Compile only, checks-only, no device: the plain XINT8 bf16 container with --no-workspace-reuse, so every tensor stays resident. Its weight packets hash the same as the plain reuse container's (40cdca73e7e4baeb), and its instruction stream the same as the AdaRound no-reuse container's (da89e12959ffd58d). Its workspace is 20.3 MB against 11.7, and its xclbin differs. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [sesr_m7_bf16_noreuse_dump_desktop2_20260923.log](sesr_m7_bf16_noreuse_dump_desktop2_20260923.log) | Silicon, checks-only, device idle either side, no foreign context: the plain no-reuse container on the six exactness frames and a repeat, with --isolate all --dump-frames, against the aligned emulator. It exits 1, as predicted. The device departs from aligned at exactly the 3 pre-registered values (bird body.1 two, woman body.3 one), with exp_sum's bits, and at no other compared value: S1. The repeat is identical. Layer by layer from the device's own inputs, over 55,050,240 values, exp_sum gets 0 wrong, aligned 3, dot_first 7, wide 8 and sequential 10. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2) |
| [sesr_m7_bf16_noreuse_replay_exp_sum_desktop2_20260923.log](sesr_m7_bf16_noreuse_replay_exp_sum_desktop2_20260923.log) | Offline (--from-dump --mac-model exp_sum --isolate none), no device: that dump chained under exp_sum. The device equals the emulator at every compared value of all 10 tensors and the tail, on all six frames (56,229,888 resident and 6,291,456 tail values); device_equals_emulator true. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2) |
| [sesr_m7_adaround_bf16_noreuse_chained_exp_sum_desktop2_20260923.log](sesr_m7_adaround_bf16_noreuse_chained_exp_sum_desktop2_20260923.log) | Offline, no device: the same replay of the AdaRound no-reuse dump, and the same result. It is exp_sum's own fitting data, so it shows that the chained replay agrees, not that the model is right. - [BENCHMARKS](../../docs/BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2) |
| [bf16_mac_rescore_exp_sum_sesr_plain_desktop2_20260923.log](bf16_mac_rescore_exp_sum_sesr_plain_desktop2_20260923.log) | Offline, no device: tools/bf16_mac_model_rescore.py sesr on the plain XINT8 SESR-M7 container and the three 2026-09-22 sittings, every frame of which was exact under aligned. The rebuild matches each sitting's hashes, inputs and per-frame value counts (5,177,344 resident, 1,048,576 tail). Over 32,636,928 scored values per 6-frame sitting (the 5,177,344 resident values plus the tail's 262,144 padding-lane values, per frame), exp_sum's bits differ from aligned's at 0, wide's at 29, dot_first's at 4 and sequential's at 186, so the last three are refuted there. In tensors the reuse container overwrites, exp_sum and aligned part at 3 values (bird 2, woman 1), which no sitting has compared. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_rescore_exp_sum_sesr_adaround_desktop2_20260923.log](bf16_mac_rescore_exp_sum_sesr_adaround_desktop2_20260923.log) | Offline, no device: the same on the AdaRound reuse container and its silicon and dump sittings. Only bird and woman were exact under aligned there; over their 10,878,976 values exp_sum differs from aligned at 0, wide, dot_first and sequential at 1 each. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [sesr_m7_adaround_bf16_dump_replay_exp_sum_desktop2_20260923.log](sesr_m7_adaround_bf16_dump_replay_exp_sum_desktop2_20260923.log) | Offline (--from-dump --mac-model exp_sum --isolate none), no device: the AdaRound reuse dump against the exp_sum emulator. Every frame equals it at the tail and every resident tensor, where aligned misses 129 resident and 11 tail values. The same weights and inputs as the no-reuse dump exp_sum was fitted to, so this is the fit's own data through a second container, not a test. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_rescore_exp_sum_sweep_desktop2_20260923.log](bf16_mac_rescore_exp_sum_sweep_desktop2_20260923.log) | Offline, no device: tools/bf16_mac_model_rescore.py sweep on the eight one-core sweep sittings (the 2026-09-21 probe and loop-variant sittings, the 2026-09-22 contract sittings). Every case label rebuilds and every logged per-model count reproduces; over 132,800 elements no model's bits differ from aligned's. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_rescore_exp_sum_16core_desktop2_20260923.log](bf16_mac_rescore_exp_sum_16core_desktop2_20260923.log) | Offline, no device: tools/bf16_mac_model_rescore.py sixteen on the 2026-09-22 Gaussian sitting. The rebuild under aligned equals that build's expected.npy byte for byte; over 153,600 elements exp_sum and wide differ from aligned at 0, dot_first at 1 and sequential at 2. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
| [bf16_mac_rescore_exp_sum_16core_integer_desktop2_20260923.log](bf16_mac_rescore_exp_sum_16core_integer_desktop2_20260923.log) | Offline, no device: the same on the integer sitting (operands 0..3, weights -1..1, bias -4..4). Every sum is an exact integer, so no model differs from aligned at any of the 153,600 elements. - [BENCHMARKS](../../docs/BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline) |
