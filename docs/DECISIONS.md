...
- **Bit-exactness to the emulator is re-established for each weight set: SESR's AdaRound weights break it where the plain weights did not (2026-09-23).**
  ...
  - *Run since (2026-09-23, Desktop 2):* two tests were pre-registered after the fit.
    ...
    - Under it, both weight sets' no-reuse dumps equal the emulator at every value of every tensor. The mechanism is therefore no longer unexplained: the core places a product at the sum of its operands' exponents ([BENCHMARKS](BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2)).
