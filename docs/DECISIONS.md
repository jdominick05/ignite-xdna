...
- **The sigmoid SiLU epilogue is in the core program, and the compiler emits it only on request (2026-09-17).**
  ...
  **It stays opt-in, not the default:**
  ...
  - AMD's accuracy on the same 500 images has not been measured, so the trade (2.4-3.9 % dispatch for 5.5-11.9
    ...
    maintainer's call.)
- **Vectorized AVX2 ingress activation staging and redundant branch elimination in preprocessing (2026-09-17).**
  Ingress activation staging in `src/ignite_xdna/pipelines/preprocess_simd.c` vectorized using 256-bit AVX2 SIMD with `/arch:AVX2`. Precomputes byte offsets in `XCoordTable` (`off0 = x0 * 3`, `off1 = x1 * 3`), evaluates paired Q11 spatial bilinear weights using `_mm256_mullo_epi32`, and writes padded letterbox borders via `_mm256_blendv_epi8`. Intermediate clipping branches (`v < 0` / `v > 255`) eliminated: for inputs in $[0, 255]$ and Q11 weights summing to 2048, bilinear products never exceed $[0, 523264]$, guaranteeing bit-exactness within $[0, 255]$ without branch overhead. Drops preprocess latency to 0.135 ms on SESR M7 (3.1× faster than AMD), 0.329 ms on YOLOv8n-pose (10.0× faster), 0.364 ms on YOLOv8n (5.2× faster), and 0.396 ms on YOLOv8s (4.8× faster). Verified 100% bit-exact across 12 test image shapes and on-device NPU inference tensors ([BENCHMARKS](BENCHMARKS.md#avx2-vectorized-ingress-activation-staging-and-q11-spatial-interpolation-2026-09-17-desktop-2)).
- **Liveness-based workspace buffer reuse with geometry-invariant halo ring preservation and relaxed DMA retirement (2026-09-17).**
  Intermediate activation tensors in `src/ignite_xdna/compiler/engine_schedule.py::plan_workspace` allocated using greedy first-fit interval coloring grouped by exact geometry key `(height, width, halo, halo_value)`. Halo Ring Invariance: intermediate layer drain patterns write strictly to the interior $(y \in [0, H), x \in [0, W))$ and never touch halo borders $(y < 0 \lor y \ge H \lor x < 0 \lor x \ge W)$, so reusing slots across disjoint lifetimes $[ \text{first\_use}, \text{last\_use} ]$ for tensors sharing exact geometry preserves intact 128 zero-point halo borders bit-for-bit without re-initialization. Cuts workspace footprint across the model zoo: SESR M7 -43.2% (19.71 -> 11.19 MB), YOLOv8s -38.8% (32.24 -> 19.73 MB), YOLOv8n -35.8% (22.04 -> 14.14 MB), YOLOv8n-pose -35.6% (22.60 -> 14.55 MB). DMA retirement in `src/ignite_xdna/compiler/engine_sequence.py::run_column_programs` relaxed from `retire_batch = 2` to `retire_batch = 4`, aligning token issuance with the 4-deep hardware shim start queue. Eliminates 30% to 46% of await instructions and completion tokens (YOLOv8s -1,656 awaits, YOLOv8n -561 awaits) and shrinks YOLOv8n's instruction stream from 430 KB to 405 KB (-24.7 KB). Verified 100% bit-exact across all output heads and intermediate layers on physical Phoenix silicon on Device 0 ([BENCHMARKS](BENCHMARKS.md#liveness-based-workspace-buffer-reuse-and-dma-retirement-relaxation-2026-09-17-desktop-2)).


- **Topology audit is opt-in refusal, not an automatic model rewrite (2026-09-17).**
  `src/ignite_xdna/compiler/topology.py` reports two model-side levers before graph
  lowering: 5x5 convolutions preserve the engine's 3x3 packet shape and therefore
  carry unread packet slack, while a stride-1 first spatial convolution keeps the
  highest-resolution activation alive into the graph. `ignite-compile` reports
  these findings by default and accepts `--topology-policy refuse` for experiments
  that require stacked 3x3 layers and early stride-2/4 patchification. It does
  not silently rewrite weights or claim an accuracy/latency benefit; those require
  a separately exported model and a logged CPU/NPU comparison.
...
- **The bf16 emulator's multiply-accumulate is measured, not assumed, and a random sweep cannot select one (2026-09-21).**
  A bf16 packet has no reference but `compiler/engine_bf16_emulator.py`, so what that file takes the core's arithmetic to be decides what "byte-exact" means. It had contracted each kernel tap in one `einsum` - an order NumPy does not define - and passed five shapes on silicon anyway, because output is compared after rounding to bf16's 8-bit significand. On the same sweep four different accumulate models all score 1,600 of 1,600. The instruction's summation (`::mac_4x8_8x4_conf`, one hardware op) cannot be read from source, so it was probed with exact powers of two built to make the candidates disagree by 0.0 against 1.0; only an **aligned** model - nine operands aligned to the largest exponent, each rounded separately to a 24-bit grid, ties to even - reproduces all 3,200 elements ([BENCHMARKS](BENCHMARKS.md#bf16-convolution-on-one-core-the-multiply-accumulate-is-an-aligned-add-every-path-the-engine-core-has-is-byte-exact-and-header-parameterisation-costs-128-2026-09-21-desktop-2)). Rule that follows: a reference for a float datapath is accepted on a probe built to break it, never on a random sweep, and the rejected candidates stay in the file so the probe keeps testing them. The comparison is on 16-bit patterns; comparing float values calls -0.0 and +0.0 equal. **What it cost the accuracy case, measured rather than argued.** The decision to build a bf16 engine rests on a CPU score (MODNet-Cut 0.5029 against FP32's 0.5030), and `tools/onnx_bf16_cast.py` had justified its float32 accumulation by asserting the core's accumulator is fp32 - which this probe disproved, demoting that score to an upper bound. It was measured back: over the tap counts real layers have, including the post-ReLU case where errors accumulate instead of cancelling, the aligned model tracks an exact sum to 2e-7 and NO lane differs after the bf16 store, because the grid keeps 24 bits below an instruction's largest operand where the store keeps 8 ([BENCHMARKS](BENCHMARKS.md#the-cores-aligned-accumulate-is-invisible-after-a-bf16-store-and-the-cpu-bf16-score-survives-it-2026-09-21-desktop-2)). The boundary is a cancellation ratio of 2**24 inside one multiply-accumulate, bracketed on both sides - and it is the grid's quantum, not the store's width, so the 2**16 predicted from the store was wrong. (Qualified 2026-09-22: "no lane" was 128 lanes a case, and on SESR-M7's own tensors the store does see the grid, rarely - 80 departures from the exact sum against 72 for an exact-per-instruction model, over six inputs. The score conclusion stands, now measured on a model: [W8A16 on the bf16 engine](BENCHMARKS.md#w8a16-on-the-bf16-engine-sesr-m7-stays-within-one-bf16-step-of-its-oracle-and-the-cores-aligned-accumulate-is-not-the-gap-2026-09-22-offline).) Rule that follows: an arithmetic finding about the datapath is not finished until it has been priced against the score that depends on it; a CPU number keeps its standing by measurement, not by the absence of an objection.
- **The `ptr` loop pattern is landed in `engine.cc`, and the YOLOv8s gap to AMD is closed (2026-09-22).**
  The port was left as a candidate with an unrun gate and one named risk: `ptr` recovered only part of the density on bf16, so whether int8's loops moved the same way was unknown until a copy was compiled. Compiled, it is better than the bf16 figure predicted - the stride-1 dual loop goes 23 bundles per 8 MACs to 18, the stride-2 dual 32 to 27, and the object **shrinks** by 96 B where bf16's cost +80, because int8's loop multiplied `c * d.plane_bytes` every iteration and a pointer increment replaces that with an add. The whole gate then passed as written: object 15,184 B under the 15,504 B budget with 0 accumulator stack moves, YOLOv8n and YOLOv8s both 66/66 byte-exact on reuse-off builds, and an interleaved same-sitting A/B at 500 frames giving -2.8% on YOLOv8n and -4.4% on YOLOv8s ([BENCHMARKS](BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2)). **`fused_stage1` was deliberately left indexing**: its trip count is the constant 2, the compiler already strength-reduces it, and a pointer-walking variant compiles byte-identical - so the lever applies to run-time trip counts only, which is the same boundary `fused_stage2`'s 9-bundle loop already marked. What this reopens: the 2026-09-16 closure of the YOLOv8s gap surveyed every TRANSPORT lever and concluded correctly about all of them, but the sentence it produced - "no runtime lever is left to close it" - was scoped to transport and read as scoped to the frame. **Rule that follows: a closure names the class it exhausted.** "Every transport lever is spent" is a finding; "nothing can close this" is a prediction, and this repo's own record says the second one costs more when it is wrong. Not landed with it: `chunk`, the bigger bf16 win at -17.7%, which costs +2,672 B against the 320 B this leaves free and needs the 5,456 B of unreachable dispatch groups compiled out first.
- **A container compiles out the dispatch groups it does not reach, and the manifest names the kernel it got (2026-09-22).**
  The four groups no lowering reaches - `OP_FUSED_CONV`, `OP_MUL`, `OP_SCALE`, `OP_POOL` - are now compiled out per container by `-DENGINE_OP_<NAME>=0`, freeing **5,408 B** and taking the engine from 320 B of program memory free to 5,728 B ([BENCHMARKS](BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2)). Nothing is deleted: every opcode, its emulation and its tests stay, and a build that passes no flags gets the whole kernel. This is the room the next opcode needs, and `chunk` (+2,672 B) is the first thing that now fits. **Rule: when a build can compile out part of itself, the source hash stops identifying the binary, and the artifact has to say so.** `kernel_sha256` hashes `engine.cc`; the gated and ungated YOLOv8n containers share it and do not share a kernel, so the manifest gained `kernel_object_sha256` (the linked object) and `kernel_ops` (the cases, by name). The same rule already has a scar in this repo from the other direction - `@iron.jit` keyed a design on a path and an mtime and served four variant arms one object - and the shape is identical: **an identifier that does not cover everything the compiler read is not an identity.** The gate is derived from the packets the container actually stores rather than from the layer kinds that produced them, because the core's failure mode for an opcode it was built without is `default:`, which emits nothing and raises nothing; `check_kernel_covers_packets` refuses that container up front, and a test fires the refusal rather than assuming it. **It is a trade, not a free win, and the first write-up of it said otherwise.** The gated and text-stripped objects are byte-identical, but those are two spellings of the same gate; against the UNGATED object one surviving stride-2 dual loop goes 27 to 28 bundles, which on AIE2 is a cycle. Measured same-sitting, that is **+0.43% of a YOLOv8n dispatch**, disjoint across two rounds - paid for a lever worth two orders of magnitude more. **Rule: byte identity between two builds retires a timing question only when one of them is the build being compared against.** The census tool now reports which loops survive, vanish and move, so the next change to this kernel is warned rather than inheriting the conclusion.
- **`engine.cc` is left alone, on measurement, and `engine_bf16.cc` with it (2026-09-21).** *(Superseded the same day by the entry below: the bf16 timing this rests on ran one kernel under five names. The program-memory, accumulator and bank-collision findings in it stand.)*
  The int8 core has 224 B of program memory free once linked, not 1,104, so nothing that grows its object can land; the accumulator-count lead was misattributed (it holds eight in registers with no spill); and the one lever the object code did show - hot-loop issue density, where a constant-trip loop in the same object runs at twice the density of the production loop - was tested on the bf16 engine, where there is program memory to spend: five arithmetic-neutral variants, all byte-exact, cut the loop's bundles by 21-33% and moved the pass time by -0.0% to +1.3% at two shapes ([BENCHMARKS](BENCHMARKS.md#five-arithmetic-neutral-rewrites-of-the-bf16-hot-loop-cut-its-bundles-by-up-to-a-third-and-the-wall-clock-did-not-move-2026-09-21-desktop-2)). That is H11's result on a second kernel. The port to `engine.cc` was gated on a bf16 win and there is none. The int8 weight/activation bank collision is real on every core and cannot be placed away with IRON's scheme or pinned-buffer levers - the allocator fails rather than use the bank with room ([BENCHMARKS](BENCHMARKS.md#the-int8-engines-weightactivation-bank-collision-cannot-be-placed-away-with-the-levers-iron-exposes-2026-09-21-desktop-2)); removing it is an ObjectFifo-free design rewrite, unmeasured and not started. What a session should do next on either engine is not another static variant but the trace-unit cycle counter, which is the only instrument that can say what the loop waits on. Also priced and not landed: the four dispatch groups no compiled container reaches cost 5,456 B of the int8 object.
- **The hot-loop lever is real on the bf16 engine, so the int8 port is open again; and a jit harness carries its kernel source in the design's key (2026-09-21; supersedes the entry above).**
  The entry above rested on two timing logs in which every engine arm ran the same kernel: `@iron.jit` keys a design on the generator's bytecode and `CompileTime` values before the generator runs, and the harness chose the kernel source inside the generator, from a global, so four variant arms resolved to cached designs of other sources. Found by auditing the cache (`tools/engine_bf16_cache_audit.py` names the source each entry compiled from) before the claim shipped, not after. Rerun with the source as a `CompileTime` value, every design's object named in the log, and ten distinct designs alive in one sitting: running pointers cut a k3 pass by **10.1%** and running pointers under constant-trip loops by **17.7%**, byte-exact on every sweep case and the accumulate probe; at k1 the same two give 6.5% and 13.7%, and hoisting the bias out of the pass 9.7% ([BENCHMARKS](BENCHMARKS.md#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2)). The static prediction from bundle counts (-14% / -22%) was three quarters right, not wrong; "H11 again" was an artifact. **What follows for `engine.cc`:** the port is gated on a bf16 win and now has one. The production int8 loops (23, 32 and 17 bundles) sit beside `fused_stage2`'s 9 in the same object, and the trigger is the same compiler-visible trip count. The `ptr` pattern cost +80 B on bf16 against 224 B free once linked, so it is the candidate; `chunk`'s +2,672 B cannot land without gating unreachable dispatch groups (5,456 B priced, not landed). The gate is unchanged and is not shortened by this result: a copy built through `design.KERNEL_SOURCE` (the manifest records `kernel_sha256`, so which kernel ran is in the artifact), object at or under 15,504 B, widened census 0, YOLOv8n 66/66 on a `--no-workspace-reuse` build, then same-sitting alternating A/B with the NOP split on YOLOv8n and YOLOv8s. Not executed in the session that found this. **What follows for `engine_bf16.cc`:** the loop goes in with the contract work, once, not as a fifth commit of variants. **Rule:** a harness that builds kernels through `@iron.jit` passes the source as a `CompileTime` value carrying its text digest, and a timing log names the object each arm ran; a log that only says which source it meant to build is not evidence about the kernel. **Not `source_files`**, which the first form of this rule offered as an equal alternative and is not one: `_hash.py` hashes each entry's path string and `st_mtime` and never its content, and passes silently when the file cannot be stat'd, so it separates two arms that differ by path but not an edit that preserves the timestamp. The same gap is wider one step out: a generator's design is also shaped by module globals and module-level helpers (`engine_bf16.py`'s `W_BYTES`, `A_BYTES`, `PSUM_FLOATS`, `whole()`), and those are in neither the key nor the build manifest, which records only files the compiler opened - so editing one and rerunning can serve a stale design with no `.cc` change to catch it. A sitting that changes a design constant runs against a fresh `NPU_CACHE_HOME`, or folds the constant into a `CompileTime` value too. The earlier logs are kept and marked, per the rule that a recorded number is never deleted.
- **A ReLU6 `Clip` is accepted only where its bound provably cannot bind, and refused everywhere else (2026-09-22).**
  `Clip` appeared nowhere in `graph_ir.py`, so every ReLU6 model was refused outright or carved onto the CPU, and the failure surfaced as `0 QuantizeLinear consumers` because the loop died on the convolution before it ever reached the `Clip` node. The lowering now takes a `Clip` as the ReLU epilogue when `lo == 0` and `hi >= 127 * s1` - that is, when `6.0` lands at or above quantum 255, where the uint8 store's saturation already performs the clamp and ReLU6 is exactly ReLU - and refuses it otherwise, naming the quantum it binds at ([BENCHMARKS](BENCHMARKS.md#the-lowering-never-matched-clip-so-every-relu6-model-was-refused-matching-it-puts-11-of-modnet-cuts-17-depthwise-convolutions-on-the-engine-2026-09-22-desktop-2)). MODNet-Cut goes from 29 engine layers to 53 and from 0 of 17 depthwise convolutions on the engine to 11, 53 of 53 engine tensors byte-exact against ONNX Runtime; MobileNetV2 (35 of 35 binding) and MiDaS-small (48 of 48) stay refused, which is correct - their clamp is doing work. **Rule: widening a lowering match means proving the new shape computes the same function, not observing that it builds.** The proof here is in two parts, and both were needed: the epilogue reproduces `max(q, 128)` with max error 0 over all 256 inputs, and the binding case is shown *unreachable* rather than merely unimplemented, since `y = t*qh` with `qh` non-decreasing in `t` cannot be constant on `[1, C]` and decreasing above it. That is what justifies a refusal instead of a silent approximation. **Second rule, the one that nearly cost the whole change: an allow-list that runs before a fallback is a second gate, and it fails silently.** `dense_regions` catches a refusal and sends the unit to the host, but its `("Conv", "Relu")` operator check ran *before* that `try`, so a `Clip` unit never called the lowering and produced no diagnostic at all - fixing `graph_ir` alone would have left MODNet-Cut byte-identical while appearing to work. The scratch survey that priced this carried the matching hazard in the other direction: it read an unresolvable `Clip` bound as `+inf`, which reports "never binds" and would have *dropped* a real clamp, so the shipped code distinguishes an absent bound from an unreadable one and refuses the second. A test asserts each refusal rather than assuming it, and an absorbed `Clip` is tracked by name rather than added to the skip list, so a stray one is still refused instead of being dropped in silence.
- **Depthwise on the engine is exact and costs 2.2x what it saves, so the off-diagonal skip is now the gate on this whole path (2026-09-22).**
  The `Clip` match put 24 of MODNet-Cut's convolutions on the engine, 11 of them depthwise - the first this engine has run. On silicon they are exact: 3,750 per-layer device checks, 75 layers across 50 pinned images, zero failures, plus 50 end-to-end ([BENCHMARKS](BENCHMARKS.md#modnet-cuts-relu6-convolutions-on-silicon-every-layer-exact-and-the-frame-117-ms-slower-2026-09-22-desktop-2)). They are also **slower than the CPU regions they replaced**: `host_ms` fell 31.90 to 23.07 ms while `npu_ms` rose 21.75 to 41.58, so the frame went 87.35 to 99.08 ms and the standing against AMD from 2.82x to **3.16x**. The AMD arm's EP report hashed identically to the previous sitting, so nothing on that side moved. **Rule: moving work onto an accelerator is a measurement, not an improvement.** The lowering was exact at every step - offline against ONNX Runtime, then per layer on silicon - and it still lost, because exactness says what the hardware computes and nothing about what it costs. **What it bought is the price tag.** Depthwise is lowered as the dense convolution whose off-diagonal taps are zero, and this container is the first in the repository to carry the consequence: 1,412 of 2,418 weight packets entirely zero, 1,311 droppable. The 19.82 ms of new NPU time is the measured cost of that redundancy, which turns the off-diagonal skip from an optimisation with no surface into the decision point for the whole dense path - and bounds it honestly, since removing all of it still leaves roughly 42 ms of floor against AMD's 31.3 ms frame. **Second rule, learned by getting it wrong here: before filing a capability as missing, run the harness that would use it.** This sitting was blocked for a day behind a claim that nothing could byte-check a dense container on silicon; `benchmarks/dense_compare.py verify` already did, more strictly than the tool that cannot (`verify_engine_container`, which re-lowers through the whole-model path and chokes on `<recipe>/host_NNN` region names). The pins were read as blocking too, and they pin the *model*, not the container - which is exactly what made this run like-for-like.
- **W8A16 is exact on silicon on SESR's plain XINT8 weights, in the tensors resident at frame end (not on its AdaRound weights, below), and design A is parked on SESR's own numbers: the bf16 tile is host-bound, and the host is numpy (2026-09-22).**
  Design B of #143 ran int8 weights on the bf16 engine with bf16 activations. On SESR-M7's plain XINT8 weights the device equals the emulator to the bit over six inputs, and it gains +0.41 dB (Set5) and +0.19 dB (Set14) over the same weights at W8A8 ([BENCHMARKS](BENCHMARKS.md#w8a16-sesr-m7-on-silicon-resident-tensors-exact-to-the-emulator-041-db-over-int8-and-the-host-egress-loses-the-tile-2026-09-22-desktop-2)). Design A - an epilogue scale so int8 values ride the bf16 engine bit-exactly, then uint8 activations in DDR - was conditional on how the bf16 frame decomposes: transfer-bound makes it a speed play, dispatch- or host-bound makes it buy bit-exactness only. Measured per tile, the frame is 0.65 ms of staging, 5.21-5.28 ms of dispatch, 0.04 ms of readback and 4.72-4.87 ms of egress, with both host ends in numpy. Host is about equal to dispatch, so **A is parked**. The next step is a native uint16 egress, which is now a measured need rather than speculation. It would leave the frame dispatch-bound, and whether that 5.2 ms is transfer or core is not separated: it is 1.27-1.30x int8's dispatch at the same 1,521 rounds and stream length, and unexplained. Two rules follow. **The host boundary is timed in the same sitting as the dispatch, never assumed free:** it decided this result. **An accuracy gain over one int8 arm is not a gain over int8:** AMD's AdaRound W8A8 still beats plain-weight W8A16 by 0.51 dB on Set5, and W8A16 on the AdaRound weights is unmeasured.
- **With native host paths the bf16 SESR frame is dispatch-bound, and design A stays parked on the frame that entry asked for (2026-09-22).**
  The native uint16 host ends named above are built in `preprocess_simd.c`, and they equal the numpy paths byte for byte. In one sitting the bf16 tile went from 10.65-10.79 ms with numpy to 6.28-6.29 ms, under the CPU's 10.75 ([BENCHMARKS](BENCHMARKS.md#w8a16-sesr-m7-with-native-host-paths-the-tile-beats-the-cpu-and-the-frame-is-now-dispatch-bound-2026-09-22-desktop-2)). Dispatch is now 84% of the tile (5.24-5.27 ms) and the two host ends 16%.
  - **By the plan's rule, a dispatch-bound frame means design A buys bit-exactness and nothing else, so A stays parked.**
  - There is also a ceiling argument, which is reasoning and not a measurement. A carries int8 values bit-exactly, so its outputs would be the int8 container's, and the int8 engine already delivers those exactly: 0 of 11,366,412 pixels differ from the QDQ model. A could only matter on SESR if the bf16 engine carried them faster than the int8 engine does. Nothing measured points that way, because the bf16 dispatch is the slower of the two.
  - **What is left to move is inside the dispatch.** bf16's 1.28-1.30x over int8 is at the same rounds and stream length, and it is unexplained. Separating the descriptor bytes from the core's pass is now the question that decides this frame.
  - Rule: **a derived total that lands is not a checked derivation.** The Phase 3 estimate of about 6.3 ms matched the measured tile only because its two assumed terms, egress and staging, were each about 0.5 ms off in opposite directions.
- **Bit-exactness to the emulator is re-established for each weight set: SESR's AdaRound weights break it where the plain weights did not (2026-09-23).**
  On the plain XINT8 weights the bf16 device equalled the emulator on six inputs, in two sittings. The AdaRound weights differ from them only in weight codes, and their container's instruction stream is byte-identical. On those weights the device departs from the emulator on 4 of the same 6 inputs, by 1 to 73 values out of 5.2 million. The difference is deterministic: a repeat frame and a second sitting reproduce every count ([BENCHMARKS](BENCHMARKS.md#w8a16-sesr-m7-on-the-adaround-weights-the-device-departs-from-the-emulator-on-4-of-6-inputs-and-its-aggregate-psnr-is-above-amds-adaround-w8a8-2026-09-23-desktop-2)). Every layer whose inputs are still resident when the frame ends, replayed from the device's own inputs, is exact. So the divergence begins in body.0 to body.4, which the workspace overwrites before the frame ends, and it is **unexplained**.
  - Rule: **a count of 0 on one weight set is not proof of the arithmetic model.** A container's exactness claim names the weights it was measured on. On SESR the four candidate accumulate models agree almost everywhere, so ordinary data barely tests which one is right.
  - Rule: **a chained check shows where a difference becomes visible, not where it starts.** Before attributing one to a layer, replay that layer from the device's own inputs (`tools/bf16_sr_silicon_check.py --isolate`).
  - The accuracy claims do not depend on this. They rest on the device-to-oracle comparison: 175 of 11,366,412 quality pixels differ, each by one level. The frame is still dispatch-bound, so design A's parking is unaffected.
  - The step that would locate the origin is a container compiled with workspace reuse off, so every layer's input stays resident. It is not built.
  - *Built since (2026-09-23):* with every tensor resident, the mismatch starts in body.1 and reaches all six inputs, not four. It does not follow addresses: a different instruction stream and workspace give the same counts in every tensor both containers keep. Replayed layer by layer from the device's own inputs, the aligned model gets 9 of 55,050,240 values wrong, all single values one bf16 step off and next to rounding ties, and the other three candidates get 14 to 17 wrong. So none of the four is the device's rule, and the mechanism is still **unexplained** ([BENCHMARKS](BENCHMARKS.md#the-adaround-mismatch-starts-in-body1-as-single-values-beside-bf16-rounding-ties-and-none-of-the-four-candidate-accumulate-models-reproduces-it-2026-09-23-desktop-2)). The plain weights' exactness is scoped the same way: their body.1 to body.3 were never compared.
  - *Built since (2026-09-23, offline):* a fifth candidate model, `exp_sum`, fits the nine and breaks nothing else in the replay, but it was chosen by those nine, and the 2026-09-21 probe cannot tell it from aligned. Rescored offline against every other engine sitting that can be rebuilt, it disagrees with none, and none of them separates it from aligned either. It is in the emulator as a candidate. `MAC_MODEL` stays `aligned` ([BENCHMARKS](BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline)).
  - *Run since (2026-09-23, Desktop 2):* two tests were pre-registered after the fit.
    - The one-core probe read A1, B1 and C1 for exp_sum.
    - A plain-weights no-reuse sitting read S1: the device departs from aligned at exactly the 3 values exp_sum predicts on weights it was not fitted to.
    - The rule below is met, so `MAC_MODEL` is now `exp_sum`.
    - Under it, both weight sets' no-reuse dumps equal the emulator at every compared value of every tensor. The nine are therefore no longer unexplained. A model that places a product at the sum of its operands' exponents predicts every value, and the four alternatives are refuted. That is a model the silicon agrees with, not an observed mechanism: any rule that gives the same bits fits as well ([BENCHMARKS](BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2)).
    - Exactness claimed under aligned keeps that scope.
  - Rule: **a model fitted to the misses goes in force only after a pre-registered probe confirms it.** The probe's predictions and the meaning of each outcome are committed before the sitting. The model goes in force only when one core reproduces the network's tiles, the nine chains read back instruction by instruction, and fresh vectors built to separate the candidates all agree with it. The layer replay must also stay exact under it.
  - Rule: **"exact" on a container with workspace reuse means exact in the tensors resident at frame end.** Differences in overwritten layers can round away before they reach one, as bird's and woman's did here. Claim whole-network exactness only from a `--no-workspace-reuse` sitting.
