...
- **A timed-out dispatch can leave every later context creation failing with `0xc01e0009` until the NPU device is restarted (2026-09-13, Desktop 2).** A DFL decode dispatch returned `ERT_CMD_STATE_TIMEOUT` after 7.02 s. Afterwards every `pyxrt.hw_context` creation failed with `0xc01e0009`, including a bare create with nothing dispatched, while `xrt-smi examine -r aie-partitions` reported no hardware contexts. A device restart, not a Windows restart (the OS boot time was unchanged), cleared it. Whether each timeout leaks one of the five context-table slots or wedges creation outright is not separated. Treat a timeout on this driver as costing a device restart: prove a new transport on its smallest shape, one dispatch per fresh context, before scaling it. [Evidence](BENCHMARKS.md#phoenix-dfl-softmax-and-anchor-decode-2026-09-13-desktop-2).
