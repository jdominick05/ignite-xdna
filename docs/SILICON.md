...
- **Local Data Memory**: 64 KB per core tile divided into 4 independent banks of 16 KB each. Simultaneous paired loads (`vlda` in `[a]` and `vldb` in `[b]`) addressing the same 16 KB bank stall the pipeline for 1 extra cycle (`MEMORY_STALL`) [MEASURED: `results/aie/bank_conflict_survey.log`, `results/aie/bank_check_validation.log`].
...
2. **bf16 GEMM: Halved Granularity Eliminates Vector Spilling** [MEASURED: `results/aie/accumulator_width_vs_count.log`, `results/aie/bank_conflict_survey.log`]:
   ...
   - Furthermore, memory allocation assigns Bank 0 to output C, Bank 1 to input A, Bank 2 to input B, and leaves Bank 3 empty. Because input operands occupy distinct 16 KB banks, paired loads execute with zero bank conflict stalls.
