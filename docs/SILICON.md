...
| int8×int4 MACs | **512 per `vmac`** (4×16×8) — native on AIE2, absent from `device.yaml`'s table below; best measured k loop 0.73–0.75 `vmac` per cycle | SPEC, and prior art: AIE-API 2024.1 lists AIE-ML's `8b x 4b: 4x16x8` as a native `mmul`, and Riallto states 512 int4×int8 MAC per cycle per core (cross-audit ledger D11). The 512 is the shape, not a count. MEASURED 2026-09-10 on one core, confirming it on Phoenix through Peano: `aie::mmul<4,16,8,int8,int4>` lowers to int8×int8's `vmac` builtin with the configuration word's B-mode field cleared and is bit-exact on hardware (B two per byte, low nibble first, two's complement; 0 of 4,096 mismatches against an int64 reference). Back-to-back issue is DERIVED from measured cycles: 8 consecutive `vmac`s run 17.03 cycles per 16-bundle loop. The best k loop sustains 372.4 MAC/cycle two-point, 383.7 fit, against int8's 204.8–227.6. AIE2's second load unit also widens int4 to int8 in the load (`vldb.unpack.s8.s4`) at no cycle cost. `results/aie/w4a8_probe_npu.log`; [BENCHMARKS](BENCHMARKS.md#int8int4-is-a-native-vmac-on-aie2-and-int4-weights-cost-nothing-to-store). At the array (MEASURED the same day, `whole_array`, 16 cores) the doubled MAC rate does not show at the int8 GEMM's best tile: packed int4 B gives 1.23–1.26× there whether the core multiplies natively or widens to int8 (6,195 GOPS); it shows at 64/64/64 (native 6.7% over unpack). `results/aie/w4a8_array_npu.log`. |
| uint8×int4 MACs | Same `vmac`, same rate: uint8 A changes only the MAC's sign bit | MEASURED 2026-09-23 on one core. `aie::mmul<4,16,8,uint8,int4>` (the graph engine's activation type) is bit-exact on hardware in 12 of 12 cells with half of A ≥ 128, and every cell's trace cycles equal int8×int4's. The one-core int8×int4 probe re-ran cycle-identical to 2026-09-10 in all 27 cells, and the array's best tile reproduced at 1.287× (int4 B over int8). `results/aie/int4_demo_npu_desktop2_20260923.log`; [BENCHMARKS](BENCHMARKS.md#int4-on-phoenix-gates-first-the-chip-runs-the-engines-uint8--int4-and-on-paper-the-current-weight-packets-leave-int4-little-to-save-2026-09-23-desktop-2). |
| int4 activations, int4×int4, int16×int4 | No aie_api/Peano dense path | TOOLCHAIN, not silicon (2026-09-23). They do not compile as `aie::mmul` for aie2, and Peano's `aiev2_vmult.h` has no MAC with a 4-bit A: its only 4-bit code (bmode 0) pairs with an 8-bit A. 8 of the 16 (amode, bmode) control-word codes are never emitted; whether the hardware decodes them is TO VERIFY. `results/aie/int4_isa_gate_desktop2_20260923.log`. |
| Adds per cycle | int8, int4: 64; bf16, int16: 32 | SPEC: `device.yaml` AIE2 `adds_per_cycle`. |
| Absent from the table | int16×int16, int8×int4, int16×int4, bfp16×bfp16 | SPEC: those appear only in the AIE2p (Strix) block. **int8×int4 is CONTRADICTED as "AIE2p only", 2026-09-23**, by two AMD sources fetched that day. SPEC(AIE-API): the AIE-API 2024.1 `mmul` modes page lists for AIE-ML "8b x 4b: 4x16x8, 8x16x8a, 4x32x8ab", where *a* marks an emulated mode and *b* one that needs extra data manipulation, so 4×16×8 is native: 512 MACs per instruction, 512 MAC/cycle per core ([group__group__mmul](https://download.amd.com/docnav/aiengine/xilinx2024_1/aiengine_api/aie_api/doc/group__group__mmul.html)). Riallto says the same: "an AIE can achieve 512 MAC/cycle when operating using int4 x int8 number formats" ([3_2_Ryzenai_capabilities](https://riallto.ai/notebooks/3_2_Ryzenai_capabilities.html)). `device.yaml`'s AIE2 table simply omits the mode. The silicon run is on `main` since the int4 merge (`5e25ddb`, 2026-09-23; commits `e91887c`, `46bc01c`, which were `56b4e88`, `2fe7624` before the branch's rebase onto `35d58d5` and message scrub: `w4a8_probe_npu.log`, `w4a8_array_npu.log`), and its MEASURED figure is the int8×int4 row above. int16×int16, int16×int4 and bfp16 are unchanged: no evidence in this tree says otherwise. No vector fp32 multiply path is listed either; fp32 *accumulation* is native (`accfloat`), and `kernels/groupnorm_bf16/groupnorm_kernels.cc` gets fp32-grade products by splitting a coefficient into a bf16 hi part and a bf16 residual — two MACs, not one. |
...
- **Accumulator Register File**: 9 physical 1024-bit registers (`cm0`–`cm8`), partitionable as 18 512-bit halves (`bml0`–`bml8`, `bmh0`–`bmh8`) or 36 256-bit quarters (`amll`–`amhh`). Holding >5 live 1024-bit accumulators triggers LLVM/Peano stack spill frames [MEASURED: `results/aie/aie2_isa_static.log`, `results/aie/accumulator_width_vs_count.log`]. **Refined 2026-09-23:** the 9 are confirmed by SPEC(Peano) and SPEC(UG1603). ">5 spills" holds for the probe's access pattern, a fresh A and B vector per MAC, and measures vector-register pressure. It is not a limit of the accumulator file: the graph engine's core names all nine and has no accumulator stack traffic (1.2, live-accumulator row).
...
MEASURED against the model, 2026-09-10 (`results/aie/w4a8_array_npu.log`): packing B as int4
gives two configurations of the same design identical bytes per MAC — int8×int4 at 64/128/64
and int8 at 128/64/64 both deliver 8 KB of A and 4 KB of B per 524,288-MAC call, 0.0234 B/MAC —
and they take 4,874–5,002 against 6,353 cycles per call (DERIVED at 1.80 GHz), 1.27–1.30×
apart where the model gives them one ceiling. Both terms named above as missing (k, and DMA run
length) differ between the two, and so does B's L3 re-stream count; this is a third data point
against the model, not the missing term.
...
- int16×int16, int16×int4 and bfp16 MACs — AIE2p only (SPEC 1.2), and untested here. They stand
  ...
  confirms it: a native 512-MAC `vmac` (SPEC, the shape), bit-exact on hardware, whose best k
  loop sustains 0.73–0.75 `vmac` per cycle (MEASURED, 1.2's int8×int4 row;
  `results/aie/w4a8_probe_npu.log`, on `main` since the int4 merge `5e25ddb`).
