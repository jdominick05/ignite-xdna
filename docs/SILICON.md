...
| Program memory | 16 KB | SPEC: `device.yaml` `core_program_memory: 16`. It holds the **linked core**, not the kernel object: MEASURED on eight existing builds, IRON's core loop and startup add a constant 880 B to `engine.cc`'s object, so the committed int8 engine (object 15,280 B) occupies 16,160 B and has **224 B free** ([`engine_linked_program_size_desktop2_20260921.log`](../results/aie/engine_linked_program_size_desktop2_20260921.log)). |
