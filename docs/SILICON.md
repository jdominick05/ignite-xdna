...
| bf16 multiply-accumulate arithmetic | One `vmac.f` (`mmul<4,8,4>::mac`) aligns the accumulator and its eight products to the largest exponent among the nine, rounds each **separately** to a 24-bit grid at that exponent (ties to even), adds them exactly and renormalises ties-to-even. bf16 x bf16 products are exact; the fp32-to-bf16 store is ties-to-even; the accumulator carries 24 bits between instructions. **Not IEEE addition**: a +1 beside +2^30 and -2^30 is lost even when the large terms cancel first. | MEASURED: [`engine_bf16_mac_model_probe_npu_20260921.log`](../results/aie/engine_bf16_mac_model_probe_npu_20260921.log) - of four candidate models only this one reproduces all 3,200 probe elements (ordered fp32 1,568 and 1,595 of 1,600; exact sum 1,460 and 1,599). Unmeasured: the sign of an exactly zero sum, Inf/NaN/subnormal operands. **Open against it:** this is the closest of four candidates, not an exact model. Replayed layer by layer from the device's own inputs on SESR-M7's AdaRound weights, it gets 9 of 55,050,240 values wrong, each one bf16 step off and next to a rounding tie, and the other three candidates get 14 to 17 wrong. The mechanism is unexplained ([BENCHMARKS](BENCHMARKS.md#the-adaround-mismatch-starts-in-body1-as-single-values-beside-bf16-rounding-ties-and-none-of-the-four-candidate-accumulate-models-reproduces-it-2026-09-23-desktop-2)). **TO VERIFY: a fifth candidate, `exp_sum`**, places a product at the sum of its operands' exponents rather than at its own leading bit. It gets 0 of those 55,050,240 wrong and equals this model on all 3,200 probe elements (DERIVED). Rescored offline, it disagrees with no other engine sitting that can be rebuilt. It was fitted to the nine, and nothing measured separates it from this model except those nine. A one-core probe is pre-registered and not run ([BENCHMARKS](BENCHMARKS.md#a-fifth-accumulate-model-exp_sum-fits-every-bf16-engine-sitting-that-could-be-rebuilt-and-the-probe-that-can-refute-it-is-pre-registered-not-run-2026-09-23-offline)). Zero mismatches on one weight set do not prove the model ([DECISIONS](DECISIONS.md#the-graph-engine-lowers-every-layer-onto-one-persistent-core-program-packets-are-fixed-size-and-the-sequencer-is-the-budget-2026-09-13), the per-weight-set exactness entry). |
