...
- **SPEC** — read from a vendor or toolchain source file already on this machine, path
  ...
  been right every time this repo has tested one, but it is not a measurement. Since
  2026-09-23 three sub-tags name the source. **SPEC(Peano)** is the machine model of the
  compiler that builds every kernel here, pinned to the installed llvm-aie commit
  (`results/aie/peano_aie2_machine_model_a36c62b9.log`, `tools/peano_isa_facts.py`).
  **SPEC(UG1603)** and **SPEC(AIE-API)** are AMD's published documentation, fetched on that
  date (URLs inline). Several of those rows were first pointed out by an external source,
  Steinert and Breuer's *Hello XDNA!* ([ISA](https://tnzr.org/xdna/isa.html),
  [XDNA1 kernel](https://tnzr.org/xdna/xdna1_kernel.html)). The row-by-row audit against it is
  `results/aie/notes_tnzr_cross_audit.md`.
...
| Program memory | 16 KB | SPEC: `device.yaml` `core_program_memory: 16`. It holds the **linked core**, not the kernel object: MEASURED on eight existing builds, IRON's core loop and startup add a constant 880 B to `engine.cc`'s object, so an engine object of 15,280 B occupies 16,160 B and has **224 B free** ([`engine_linked_program_size_desktop2_20260921.log`](../results/aie/engine_linked_program_size_desktop2_20260921.log)). *(Updated 2026-09-22: the committed engine is now the 15,184 B ptr build, so 16,064 B used and **320 B free** - [the ptr port](BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2).)* *(And again 2026-09-22: a container now compiles out the four dispatch groups it cannot reach, so a shipped object is **9,776 B** with **5,728 B free** - [the dispatch gate](BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2). The ungated figure stands for a build that passes no gate flags.)* MEASURED (static, 2026-09-23): the graph engine's current core ELF (`kernels/aie2/conv_engine/engine.cc`, shared by 56 of 68 builds under `build/conv_engine/`) has a `.text` of **16,160 of 16,384 B, 224 B free**. One experimental variant (`yolov8n_stride2ctrl`) sits at 16,368 B, and `yolov8n_full`, the build behind `build/yolov8n_full.ignite`, is the older 10,704-B version. Across the 68 builds there are four distinct core ELFs, so "one fixed core program" holds per engine version, not across the tree (`results/aie/engine_core_issue_census_desktop2_20260923.log`, `tools/engine_core_census.py`). *(Scope, added on merging `main` into `worktree-bf16-engine`, 2026-09-23: "current" there means `main`'s core before the [ptr port](BENCHMARKS.md#the-ptr-loop-pattern-lands-in-the-int8-engine-and-yolov8s-overtakes-amds-stack-2026-09-22-desktop-2) and the [dispatch gate](BENCHMARKS.md#compiling-out-the-dispatch-no-container-reaches-frees-5408-b-and-costs-a-manifest-field-to-stay-honest-2026-09-22-desktop-2) above. None of the four ELFs it read is a `ptr` build: they link at 16,160, 16,048, 16,368 and 10,704 B, and a `ptr` build occupies 16,064 B (its 15,184 B object plus the constant 880 B). Its 16,160 B equals the 2026-09-21 linked figure. The census has not been run on a build of the merged `engine.cc`.)* |
| MACs per cycle | int8×int8 **256**; bf16×bf16 **128**; int16×int8 **128** | SPEC: `device.yaml` AIE2 `macs_per_cycle`. |
| bf16 multiply-accumulate arithmetic | One `vmac.f` (`mmul<4,8,4>::mac`) places the accumulator at its own exponent, and each of its eight products at the **sum of its operands' exponents** (ea + ew), not at the product's leading bit. It aligns the nine to the largest of those places, rounds each **separately** to a 24-bit grid there (ties to even), adds them exactly and renormalises ties-to-even. This is `exp_sum` in the emulator, in force since 2026-09-23. Two significands multiply to [1, 4), so a product in [2, 4) keeps one more bit below its place than it would aligned at its leading bit. Where every product is under 2 this is the older `aligned` model. bf16 x bf16 products are exact; the fp32-to-bf16 store is ties-to-even; the accumulator carries 24 bits between instructions. **Not IEEE addition**: a +1 beside +2^30 and -2^30 is lost even when the large terms cancel first. | MEASURED 2026-09-23. The model was fitted to 9 values `aligned` misses on SESR-M7's AdaRound weights ([BENCHMARKS](BENCHMARKS.md#the-adaround-mismatch-starts-in-body1-as-single-values-beside-bf16-rounding-ties-and-none-of-the-four-candidate-accumulate-models-reproduces-it-2026-09-23-desktop-2)), then passed two tests pre-registered after the fit. (1) A one-core probe ([`bf16_mac_position_probe_npu_desktop2_20260923.log`](../results/aie/bf16_mac_position_probe_npu_desktop2_20260923.log)): one core equals the network on 10 of 10 tiles, and `exp_sum` predicts 132 of 132 readouts, against `aligned`'s 25 ([BENCHMARKS](BENCHMARKS.md#the-one-core-probe-measures-exp_sum-a1-b1-and-c1-on-the-pre-registered-plan-2026-09-23-desktop-2)). (2) A plain-weights no-reuse sitting ([`sesr_m7_bf16_noreuse_dump_desktop2_20260923.log`](../results/aie/sesr_m7_bf16_noreuse_dump_desktop2_20260923.log)): the device departs from `aligned` at exactly the 3 values `exp_sum` predicts, and nowhere else. Layer by layer from the device's own inputs, `exp_sum` gets 0 of 55,050,240 values wrong on each weight set ([BENCHMARKS](BENCHMARKS.md#the-plain-weights-no-reuse-sitting-reads-s1-the-device-departs-from-aligned-at-exactly-exp_sums-3-values-2026-09-23-desktop-2)). *Superseded:* `aligned`, which places a product at its leading bit. It was MEASURED 2026-09-21 on 3,200 probe elements ([`engine_bf16_mac_model_probe_npu_20260921.log`](../results/aie/engine_bf16_mac_model_probe_npu_20260921.log)), the only one of four candidates to reproduce all of them (ordered fp32 1,568 and 1,595 of 1,600; exact sum 1,460 and 1,599). `exp_sum` equals it on all 3,200 (DERIVED). `aligned` misses 9 values on the AdaRound weights and 3 on the plain. `exp_sum` is consistent with every bf16 sitting on record, which does not prove it is the whole rule. Unmeasured: the sign of an exactly zero sum, Inf/NaN/subnormal operands. Zero mismatches on one weight set do not prove the model ([DECISIONS](DECISIONS.md#the-graph-engine-lowers-every-layer-onto-one-persistent-core-program-packets-are-fixed-size-and-the-sequencer-is-the-budget-2026-09-13), the per-weight-set exactness entry). |
...
| Accumulator cascade to a neighbour | 512 bits | SPEC: `getAccumulatorCascadeSize() = 512`. Exercised on this chip by `02_vector_reduce_max`'s 4-core cascade (`results/aie/mlir_aie_examples_npu.log`). Formally modelled 2026-09-10 ([`results/aie/notes_accumulator_cascade_gemm_model.md`](../results/aie/notes_accumulator_cascade_gemm_model.md)): 4-core vertical K-reduction eliminates intermediate C-tile L1 traffic (1.94× input volume) and paired-load bank conflicts, enabling wide tiles (bf16 128×64×64, int8 128×64×128) and projecting 2.16× throughput uplift (4368 → ~9450 GOPS). |
...
| Issue width | 6 slots per bundle: **`b` and `a` are the two load units**, `s` store, `x` scalar and control flow, `m` move/broadcast, `v` vector | MEASURED 2026-09-09 (`results/aie/aie2_isa_static.log`, corrected by `results/aie/bank_conflict_survey.log`). **Supersedes this row's earlier "`b` branch"**, which was inferred from the nop mnemonic alone. Tabulating every operation appearing in each slot of 226 *strictly six-field* bundles — the only encoding whose slot identity is unambiguous — puts `vldb` and `paddb` in slot b, `vlda`/`lda`/`mova` in slot a, and `ret` in the scalar slot x. So the core has **two load units**, which is what makes a same-bank paired load possible at all. `nopxm` is the fused encoding when x and m are both idle, so a five-field bundle still occupies six slots; bundles using few slots are emitted compressed and shorter than 16 B but still issue in one cycle. **Confirmed and extended 2026-09-23, SPEC(Peano)** (`results/aie/peano_aie2_machine_model_a36c62b9.log` §2–§3): the compiler's own slot names are lda, ldb, alu, mv, st and vec. **`vshift`, `vshift.align`, `vshuffle`, `vbcst` and register `vmov` issue in the mv slot, not in vec**, where `vmac`/`vmul` go. So they co-issue with a `vmac`, which the repo's own disassembly already showed (`results/aie/conv_issue_rate_decomposed.log`: `0x072a vmov | vmac`, `0x075c vshift | vmac`, `0x0780` with `vldb`, `vlda`, `add`, `vshift` and `vmac` in one bundle). |
| Local memory banks | 64 KB of core-tile data memory in **4 banks of 16 KB**. Two loads issued in one bundle cost **one extra cycle** when both address the same bank | SPEC: `getLocalMemorySize()` 0x10000 and `getNumBanks()` 4 for a core tile in mlir-aie's `AIETargetModel.h`. MEASURED 2026-09-09, first on branch `research/windows-lowlevel` and now in-tree (`results/aie/memory_desktop2_20260909_m01_*.log`, eleven logs), holding the compiled function bytes identical and changing only operand addresses: two loads same bank 12.0 cycles/iteration, separate banks 11.0, one load same bank 11.0, r² 1.0. Operand offsets of 64/128/256 B within a bank make no difference, so the granularity is the bank. Buffer addresses are not in the cached `aie.mlir`, which is pre-allocation; they are in the core ELF's symbol table (`tools/aie_bank_check.py`). Corroborated by a second, independent instrument: `results/aie/bank_stall_observable_npu.log` counts the stall events themselves inside the dispatch rather than fitting cycle slopes, and finds one same-bank **paired** load raises exactly one `MEMORY_STALL` and costs exactly one cycle (514/1026/2050/4098 events at trip counts 512/1024/2048/4096, against a constant 2 when separated). The pairing is the condition: a loop too loose for the compiler to bundle the two loads pays nothing. **A load and a store to one bank in one bundle cost the same +1 cycle** (MEASURED 2026-09-23, `results/aie/l1_tile_mm_i8_k64_c_bank0_desktop2_20260923T0545Z.log` against `results/aie/l1_tile_mm_k64_desktop2_20260923T0533Z.log`). The reference states the rule ("loads and stores in separate instructions to avoid bank conflicts", [Hello XDNA!](https://tnzr.org/xdna/xdna1_kernel.html)), and this repo had measured only load+load. Upstream `mm.cc` int8 spills its accumulators to the stack, which lives in bank 0, and its per-block epilogue has exactly **12** bundles pairing a stack reload (`vlda am.., [sp, ..]`) with a C store (`vst am.., [p3/p4/p5]`). With C moved into bank 0 beside the stack, the same kernel in the same harness costs 745.6 cycles per 32×64×32 call against 697.1. That is **+48.5 measured against 4 blocks × 12 × 1 = 48 predicted**. The bf16 kernel has no such bundle and does not move (1418.5 vs 1418.6). `tools/aie_bank_check.py` cannot see this hazard: it drops the stack from its bank map and pairs only load with load. |
| Cycles per hardware-loop iteration | equal to the loop body's bundle count | MEASURED 2026-09-09: the core is a statically scheduled VLIW with an exposed pipeline, so Peano covers operand latency with explicit nop bundles rather than an interlock. S0's two loops measured 9.000 and 2.000 cycles/iteration and disassemble to 9 and 2 bundles (`results/aie/aie2_isa_static.log`). An inner loop's cost is therefore readable before the kernel runs; a loop that waits on a lock, stream or DMA is the exception and needs the trace unit. **A fourth exception, added 2026-09-09:** a bundle issuing two loads whose addresses share a 16 KB bank costs one extra cycle, so the law holds for a loop whose simultaneous loads are in *different* banks (`results/aie/bank_conflict_survey.log`). The production int8 GEMM violates this and its 9-bundle loop should cost 10. **Confirmed on a convolution kernel 2026-09-21:** the bf16 engine's hot loop rewritten from 24 to 19 and to 16 bundles per 8 multiply-accumulates, arithmetic unchanged, cut a k3 pass by 10.1% and 17.7% against a bundle-count prediction of 14% and 22% (`results/aie/engine_bf16_loop_variants_bench_k3_npu_20260921_02.log`); an earlier log reading "did not move" had timed one kernel under five names. **And on the int8 engine 2026-09-22:** the same rewrite took its stride-1 dual loop from 23 bundles per 8 multiply-accumulates to 18 and its stride-2 dual from 32 to 27, byte-exact, and moved a whole frame by -2.8% on YOLOv8n and -4.4% on YOLOv8s (`results/aie/ptr_ab_yolov8s_phoenix_20260922T0317Z.log`). The law reads the loop, not the frame: a 21.7% saving in a loop that is a fraction of a transport-bound frame arrives as a fraction of 21.7%. **Checked on silicon 2026-09-23 with operands resident in L1** (`tools/l1_tile_bench.py`, every output exact). A hand-scheduled kernel with no same-bank pair runs at exactly its bundle count: *Hello XDNA!*'s bf16 32×32×32 tile costs 282.0 cycles per call over an empty-call control. That is its 288 executed bundles less the control's own 6 executed ones (`kernels/l1_tile_bench/empty_call.s`: `ret lr` + 5 delay slots; 7 static bundles, the seventh a `.p2align` pad), so zero stalls (`results/aie/l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log`). Upstream `mm.cc` loops cost their bundle count **plus one** per iteration: each extra K adds 32.5–33.3 cycles per bf16 iteration against 32 bundles, and 9.8–10.2 per int8 iteration against 9 (`results/aie/l1_tile_mm_k64_desktop2_20260923T0533Z.log`, `…_k128_…`). Each loop body has exactly one paired load, and both of its loads read **one operand buffer**: int8 `vlda [p4], #0x20` + `vldb [p3], #0x20` are two A pointers, and bf16's pair is two B loads. So the pair shares a bank wherever A, B and C are placed. It was measured with A, B and C in three separate banks. Attribution: the only stall source left with no interlocks and no DMA in the loop, but not isolated, because no placement can separate them. |
...
| Operation latencies | VLDA/VLDB **7**; ST/VST **1**; `vshift`/`vshuffle`/`vmov` **2**; scalar MUL **2**; `ret lr` has **5 delay slots**. Accumulator load → first `vmac.f` 5 bundles; `vmac.f` → store 6; int8 `vmac` → store 5 | SPEC(Peano), 2026-09-23 (`results/aie/peano_aie2_machine_model_a36c62b9.log` §3 itineraries, §4 Peano's own spacing of compiled AIE-API snippets). Agrees with *Hello XDNA!*'s ISA Table 2, and Peano's schedule of a bf16 `mmul` is bundle for bundle the reference's own listing. The scalar-load row above is the MEASURED instance of the 7. `kernels/asm_probe/asm_core_id.s`'s five NOPs after `ret lr` are those delay slots. |
| MAC latency and accumulator forwarding | bf16 `vmac.f` **6** cycles, accumulator operand read in cycle 3; int8 `vmac` **5**, accumulator bypassed (`VEC_Bypass`). Peano issues a dependent `vmac.f` **4** bundles after its producer, and a dependent int8 `vmac` **2** after | SPEC(Peano), 2026-09-23 (§3 `II_VMACf [6,3,…]`, `II_VMAC [5,3,…]` with VEC_Bypass; §4 the spacing Peano emits). DERIVED: one `vmac` per cycle needs ≥4 independent accumulators in bf16 and ≥2 in int8. Every kernel here that holds 8 is not latency-bound on its accumulators. MEASURED 2026-09-23, Desktop 2 ([vmac_chain_forwarding_desktop2_20260923T0602Z.log](../results/aie/vmac_chain_forwarding_desktop2_20260923T0602Z.log), probe P2 in `results/aie/notes_tnzr_cross_audit.md`): a hand-assembled chain of 8 dependent MACs on one accumulator lands all 8 only at an issue distance of **≥4 bundles (bf16) and ≥2 (int8)**. Closer, it lands 2 or 4 of 8 (bf16 d = 1, 2, 3 → 2, 4, 4; int8 d = 1 → 4), exactly as if each MAC read the newest sum from at least 4 (or 2) bundles back. Each variant costs its executing bundles over an empty control to within 0.3 cycles. **The core does not stall on this hazard. It computes a wrong answer.** The spacing is what is MEASURED. Its split into latency and read cycle (6 and 3 for bf16; 5 with a bypass for int8) stays SPEC(Peano). |
| Accumulator loads and stores | only load unit A (slot a, `vlda`) writes an accumulator, and at most one accumulator load or store per bundle | SPEC(Peano), 2026-09-23 (§3): every accumulator `vlda` (`II_VLDA_AM`, `II_VLDA_CONV`) reserves `LOAD_UNIT_A`; `vldb` has no accumulator form; accumulator stores issue in slot st. Consistent with every disassembly in this repo, and stated by *Hello XDNA!*. |
| Hardware-loop setup | `movxm ls`/`movxm le`/`add.nc lc` before the loop. First and last loop bundles 16-B aligned; the last bundle 16 B wide | MEASURED (static) 2026-09-23 over every loop of all four graph-engine core ELFs (`results/aie/engine_core_issue_census_desktop2_20260923.log`): the alignment and last-bundle rules hold, as *Hello XDNA!* states. Its other rule, setup **≥64 B before the loop start**, is not what the hardware needs. The engine's conv loops write `ls`/`le`/`lc` as little as **16 B** before the start (16–58 B across them) and run bit-exact. Every loop's setup is **≥112 B before the loop end** (exactly 112 in many), which suggests an end-anchored rule. TO VERIFY by a setup-distance sweep; that run carries a device-hang risk and is scheduled last (probe P3). |
| Live accumulators | **5** concurrently-live 4×8×8 int8 `aie::mmul` accumulators compile with no stack traffic; 6 is the first count that spills. The allocator names 9 accumulator registers, `cm0`–`cm8` | MEASURED 2026-09-09 by sweeping the count and reading the object code (`kernels/acc_spill_probe/`, `results/aie/aie2_isa_static.log`). Supersedes this row's earlier "≤4 stays in registers" and `docs/DECISIONS.md`'s "only 6 hardware accumulator registers"; both were inferred from the one `conv2dk3` kernel that spilled at 8. One shape and one optimisation level. The production int8 GEMM demonstrates the ceiling — it holds 8 live `mmul` accumulators, uses all 9 names, and spills, with a 416-byte frame and 33 stack references (`results/aie/gemm_cost_model.log`). **Refined 2026-09-09 (`results/aie/accumulator_width_vs_count.log`):** the file is **9 registers addressable at three granularities** — full 1024-bit `cm0`–`cm8`, 512-bit halves `bml`/`bmh`, 256-bit quarters `amll`…`amhh` — so "9 names" is the file, not a lower bound on it. And **the ceiling is not a width budget**, which this row previously implied by saying a wider accumulator fits fewer: bf16's `matmul_vectorized_4x4` holds 16×512-bit for the *same* 8192-bit total across the *same* 8 of 9 registers, and spills **nothing** — a 64-byte frame whose 15 stack references are all scalar, against int8's 12 vector spills. int8's 4×2 blocking is the defect, not the width. **Confirmed from three more sides 2026-09-23.** SPEC(Peano) defines exactly `cm0`–`cm8`, each `[bml_i, bmh_i]`, plus nine each of `amll/amlh/amhl/amhh` (`results/aie/peano_aie2_machine_model_a36c62b9.log` §1). SPEC(UG1603) documents `am0`–`am8`/`bm0`–`bm8`/`cm0`–`cm8` ([UG1603 2026.1, Accumulator Registers](https://docs.amd.com/r/en-US/ug1603-ai-engine-ml-kernel-graph/Accumulator-Registers)). By use, every engine core ELF names the ninth accumulator 26 times and runs bit-exact on silicon (`results/aie/engine_core_issue_census_desktop2_20260923.log`). *Hello XDNA!*'s "sixteen available" 512-bit accumulators understates the file. **The "5 is the spill-free ceiling" reading is REFINED:** 5 is where *this probe's shape* starts spilling, with a fresh A and B vector per MAC, so it measures vector-register pressure, not the accumulator file. The graph engine's core names all nine accumulators and has **no accumulator stack traffic**: 0 of its 169 stack references move an `am`/`bm`/`cm` register, while 31 move vector registers (`w`/`x`). That is a static reading of `build/conv_engine/modnet_cut_dense_20260921/design.prj/engine.o`, 2026-09-23. Read "5" as the ceiling for that access pattern, not for the core. |
...
| Hand-written assembly | **Assembles and links.** A standalone `.s` never enters instruction selection, so it reaches the integrated assembler intact | MEASURED 2026-09-09 (`kernels/asm_probe/`, `results/aie/aie2_isa_static.log`). Only statement-level inline asm inside a C++ function fails, in the IRTranslator, which is the wall S0 hit. Hand-scheduling an inner loop is therefore available where the compiler's schedule is the binding constraint. Toolchain result: the object assembles, disassembles and links, but no hand-written kernel has been run on the NPU. **Superseded 2026-09-23: a hand-written kernel runs, at 86% of peak.** *Hello XDNA!*'s hand-scheduled bf16 32×32×32 tile (fetched to `scratch/`, never vendored, since its repository has no licence) was assembled by this repo's Peano, linked with `link_with` through aiecc v1.4.2 and dispatched through `XrtSiliconHarness`. It ran at **397.5 GFLOPS, 86.3% of the 460.8 per-core bf16 peak**, 99.9% of the reference's 398, in `default` pmode where the reference used `turbo` (MEASURED `results/aie/tnzr_bf16_32x32x32_repro_desktop2_20260923T0518Z.log`, `tools/tnzr_repro.py`). In this repo's own L1 harness it runs exactly its static schedule, with an exact output (`results/aie/l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log`; see 2.3.2). |
| `aie::mmul` shapes in mlir-aie's GEMM library | bf16 4×8×4; int8 4×8×8; int16 4×4×4 | SPEC: `aie_kernels/aie2/mm.cc`, and the compiler's own dispatch table `_MM_MAC_DIMS` in `python/iron/kernels/linalg.py`. One AIE2 int8 `vmac` therefore retires 4·8·8 = 256 MACs, which is exactly the 256 MACs/cycle nameplate. The same table gives AIE2P (Strix) 8×8×8 for int8, i.e. 512 per `vmac` — the generational difference is the MAC shape, not the issue rate. **Added 2026-09-23.** The only *native* bf16 matmul on XDNA1 is 4×8×4. 8×8×4, 4×16×4 and 8×8×8 bf16 are emulated or need extra data manipulation, and fp32 matmul is emulated on bf16 ([Hello XDNA!, ISA Table 1](https://tnzr.org/xdna/isa.html)). The `vmac` configuration register carries the shape. SPEC(Peano): Peano loads **28** for bf16 4×8×4 and **776** for int8 4×8×8 with a 32-bit accumulator (`mova r0, #28` / `mova r0, #776`, `results/aie/peano_aie2_machine_model_a36c62b9.log` §4). The int8 × int4 4×16×8 mode is in the "Absent from the table" row above. |
| MAC issue rate, production int8 GEMM | **107.3 MACs/cycle** over a whole kernel call, 41.9% of the 256 nameplate; 88.9% inside the hardware loop alone | MEASURED 2026-09-09 by walking the compiled object's control flow and dividing the tile's required MACs by the cycles the nest costs (`tools/gemm_cost_model.py`, `results/aie/gemm_cost_model_nest.log`). Supersedes the same day's first reading of **198.1 / 77.4%**, which modelled `matmul_i8_i32` as one hardware loop with straight-line setup; it is a nest, two software loops around the hardware loop, re-running the accumulator load/store body once per group. The gap between 88.9% and 41.9% is those 87 non-loop bundles per group — accumulator load, store and stack spill — and it follows directly from the row above: this kernel holds 8 live accumulators where 5 is the spill-free ceiling. A computed issue rate, not a hardware-counter reading; the branch targets are unresolved relocations in the object, so the evidence for the nest reading is that its trip counts reconcile exactly against the required `vmac` count. Charging the same-bank paired load this kernel is now known to carry lowers it further to **103.3 MACs/cycle, 40.3%** (`results/aie/bank_conflict_survey.log`). **Measured on silicon 2026-09-23, operands resident in L1:** the same upstream kernel at 64×64×64 costs 2,604.8 cycles per call, **39.3% of peak** (362.3 GOPS per core; 39.6% over the empty-call control). That lies within 1 point of the computed 40.3%. The cost model's 2,442 / 2,538 / 2,682 cycles per call (no paired load charged / the loop's charged / every one charged, `bank_conflict_survey.log`) bracket the measured 2,584.4 over the control, so the static model holds on silicon to within that bracket (`results/aie/l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log`). **Refined 2026-09-23:** "5 is the spill-free ceiling" is that probe's vector-register pressure, not the accumulator file (see the live-accumulator row). The hot loop's paired load is two **A** pointers (`[p4], #0x20` and `[p3], #0x20`, both stepping one 4×8 A tile), so moving A and B into different banks would not remove it. |
...
| Shim task ABI | a start-queue push carries the buffer-descriptor id in its low 4 bits; the rest of the word is per-task metadata (locks, and a flag at bit 31) that this reading does not decode. Descriptor slots sit 32 bytes apart from 0x1D000, and a slot is reusable only after its task retires | MEASURED: decoded from the engine's own emitted stream by `tools/shim_channel_audit.py`, [`results/aie/shim_channel_utilisation_sesr_yolov8n_desktop2_20260919.log`](../results/aie/shim_channel_utilisation_sesr_yolov8n_desktop2_20260919.log), Desktop 2. All 1,007 descriptor writes pair with all 1,007 pushes with none naming an unwritten slot, and the reconstruction checks itself three independent ways: the 36 weight tasks sum to exactly 28 x 9,472 + 8 x 18,944 B, the 261 drain tasks to exactly 261 x 3,200 = 835,200 B, and the whole frame to the descriptors' own word-0 total, 13,181,568 B. Read with the descriptor-word encoding in 1.3, which is the mem-tile analogue of the same structure. |
...
### 2.3.1 Systematic VLIW instruction-mix and accumulator-spill census

To isolate why open AIE2 compute kernels deviate from hardware nameplate limits, the compiled machine code for eight core kernel variants across `kernels/` and `results/aie/` was audited at the VLIW bundle and instruction-issue level. The core tile features a 6-issue statically-scheduled VLIW architecture with exposed pipelines and no hardware interlocks [SPEC: `AIETargetModel.cpp`]. Each bundle can issue up to six concurrent operations across distinct execution units [MEASURED: `results/aie/aie2_isa_static.log`, corrected by `results/aie/bank_conflict_survey.log`]:
- **`[b]` Load Unit 2**: Vector and pointer loads (`vldb`, `paddb`, `nopb`).
- **`[a]` Load Unit 1**: Vector, scalar, and move loads (`vlda`, `lda`, `mova`, `nopa`).
- **`[s]` Store Unit**: Vector and scalar stores (`vst`, `st`, `nops`).
- **`[x]` Scalar ALU & Control**: Address arithmetic, comparison, loop branches, control events (`add`, `sub`, `mul`, `lshl`, `or`, `ret`, `event`, `nopx`).
- **`[m]` Move & Broadcast**: Intra-register shuffling, vector broadcasts, moves (`mov`, `vmov`, `vbcst`, `vshuffle`, `add.nc`, `nopm`).
- **`[v]` Vector Compute / MAC**: Vector arithmetic and matrix multiply-accumulate (`vmac`, `vmac.f`, `vshift`, `nopv`). Idle `[x]` and `[m]` slots are emitted as fused `nopxm`.
- **CONTRADICTED 2026-09-23: `vshift` is not a `[v]` operation.** SPEC(Peano) puts `vshift`, `vshift.align`, `vshuffle`, `vbcst` and register `vmov` in the **move slot `[m]`** (Peano's `mv`); only `vmac`/`vmul` and vector arithmetic are `[v]` (`results/aie/peano_aie2_machine_model_a36c62b9.log` §2–§3). This repo's disassembly shows them co-issuing with `vmac` (1.2, issue-width row). The list above is kept as written.

Physical execution ceilings per core tile [SPEC: `device.yaml`]:
- **int8 MAC ceiling**: 256 MACs/cycle (via 1 `vmac` executing a 4×8×8 matrix multiply-accumulate).
- **bf16 MAC ceiling**: 128 MACs/cycle (via 1 `vmac.f` executing a 4×8×4 matrix multiply-accumulate).
- **Local Data Memory**: 64 KB per core tile divided into 4 independent banks of 16 KB each. Simultaneous paired loads (`vlda` in `[a]` and `vldb` in `[b]`) addressing the same 16 KB bank stall the pipeline for 1 extra cycle (`MEMORY_STALL`) [MEASURED: `results/aie/bank_conflict_survey.log`, `results/aie/bank_check_validation.log`]. A load and a store to one bank in one bundle cost the same +1 [MEASURED 2026-09-23: `results/aie/l1_tile_mm_i8_k64_c_bank0_desktop2_20260923T0545Z.log`; 1.2, bank row].
- **Accumulator Register File**: 9 physical 1024-bit registers (`cm0`–`cm8`), partitionable as 18 512-bit halves (`bml0`–`bml8`, `bmh0`–`bmh8`) or 36 256-bit quarters (`amll`–`amhh`). Holding >5 live 1024-bit accumulators triggers LLVM/Peano stack spill frames [MEASURED: `results/aie/aie2_isa_static.log`, `results/aie/accumulator_width_vs_count.log`]. *(Scoped 2026-09-21: that ceiling was measured on `kernels/acc_spill_probe`'s k-reduction loop and on upstream GEMM, and it is a property of those loop shapes, not of the register file. The graph engine's conv pass holds **8** live 1024-bit accumulators in `cm0`–`cm7` with no accumulator stack move anywhere in the object and no stack move of any register class inside a hardware loop [MEASURED, static: `results/aie/engine_census_widened_desktop2_20260921.log`]. Five is where that probe spills; it is not a limit to design a kernel around.)* **Refined 2026-09-23:** the 9 are confirmed by SPEC(Peano) and SPEC(UG1603). ">5 spills" holds for the probe's access pattern, a fresh A and B vector per MAC, and measures vector-register pressure. It is not a limit of the accumulator file: the graph engine's core names all nine and has no accumulator stack traffic (1.2, live-accumulator row).

#### Comparative Kernel Census Table

| Kernel Variant | Precision & MMUL Unit | Loop Nest Structure & Cycle Count | Bundle Issue Breakdown `[b] [a] [s] [x] [m] [v]` | Hot-Loop `vmac`/cyc (% loop ceil) | Whole-Call Rate (% tile ceil) | Live Acc / Reg File Allocation | Stack Frame & Vector Spills | Paired Same-Bank Load Hazard | Epistemic Tag & Primary Evidence |
|---|---|---|---|---|---|---|---|---|---|
| **int8 GEMM default** (m64 k64 n32, 4×2) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop: 9 bundles (6 iters). Outer nest: 8 acc groups × 141 cyc (54 loop + 87 non-loop) | Loop (9 bundles): 4 `vldb`, 4 `vlda`, 0 `vst`, 0 `[x]`, 9 `vmov`/`mov`, 8 `vmac` | 0.889 `vmac`/cyc (88.9%) | 100.4 MAC/cyc (39.2% of 256) | 8 live 1024-bit accs; names all 9 `cm0`–`cm8`, 12 vec `x0`–`x11` | 416 B frame; 37 stack refs (12 vector spills = 384 B) | 1 in HW loop, 10 in nest. Operands A & B share Bank 2 (COLLIDE: +1 cyc/iter) | [MEASURED] `results/aie/gemm_cost_model_nest.log`, `results/aie/bank_conflict_survey.log` |
| **int8 GEMM tuned** (m64 k64 n64, 4×2) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop: 9 bundles (6 iters). Outer nest: 16 acc groups × 141 cyc; 2,442 cyc/call total | Loop (9 bundles): 4 `vldb`, 4 `vlda`, 0 `vst`, 0 `[x]`, 9 `vmov`/`mov`, 8 `vmac` | 0.889 `vmac`/cyc (88.9%) | 107.3 MAC/cyc (41.9% of 256; 103.3 MAC/cyc w/ hazard) | 8 live 1024-bit accs; names all 9 `cm0`–`cm8`, 12 vec `x0`–`x11` | 416 B frame; 33 stack refs (12 vector spills = 384 B) | 1 in HW loop (0x0200), 10 in nest. A & B in Bank 2, Bank 3 empty (COLLIDE) | [MEASURED] `results/aie/gemm_cost_model_nest.log`, `results/aie/bank_check_validation.log` |
| **int8 GEMM reblocked H11** (m64 k64 n64, 2×2) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop: 8 bundles (6 iters). Outer nest: 32 acc groups (2×2 blocking) | Loop (8 bundles): 3 `vldb`, 4 `vlda`, 0 `vst`, 0 `[x]`, 8 `vmov`/`mov`, 8 `vmac` | 1.000 `vmac`/cyc (100.0%) | 256.0 MAC/cyc in loop (100.0% of 256 tile ceil) | 4 live 1024-bit accs; names 8 `cm0`–`cm7`, 9 vec `x0`–`x6, x8, x10` | 32 B frame; 5 stack refs (0 vector spills, 100% eliminated) | Paired loads eliminated from hot loop; zero stack traffic in loop | [MEASURED] `results/aie/gemm_reblock_h11_npu.log` |
| **bf16 GEMM 4×4** (m64 k64 n32/n64) | bf16→fp32, `mmul<4,8,4>` (128 MAC/op) | HW loop: 32 bundles. Steady-state loop issues 16 `vmac.f` ops | Loop (32 bundles): 5 `vldb`, 5 `vlda`, 0 `vst`, 1 `mov`, 0 `[m]`, 16 `vmac.f` | 0.500 `vmac`/cyc (50.0%) | 64.0 MAC/cyc in loop (50.0% of 128 tile ceil) | 16 live 512-bit accs (`bml0`–`7`, `bmh0`–`7` in 8 `cm0`–`cm7`) | 64 B frame; 33 stack refs (0 vector spills, all scalar) | 1 in HW loop (0x02b8), 7 total. Bank 0=C, 1=A, 2=B, 3=empty (SEPARATED, 0 stall) | [MEASURED] `results/aie/accumulator_width_vs_count.log`, `results/aie/bank_conflict_survey.log` |
| **conv2dk1 stock** (1×1 conv, int8) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop (.L_LEnd1): 22 bundles (20 live ops, 6 all-nop bundles). Trip count W/4 | Loop (22 bundles): 1 `vldb`, 4 `vlda` (quarters), 4 `vst` (quarters), 6 `[x]`, 4 `[m]`, 1 `vmac` | 0.045 `vmac`/cyc (4.5%) | 11.6 MAC/cyc in loop (4.5% of 256 tile ceil) | 4 accs in `acc_tmp[4]` array; compiler forces memory residency | Frame spills every MAC; 128 B load + 128 B store memory round-trip per MAC | 1 paired load in loop (0x024c); high bus and memory port contention | [MEASURED] `results/aie/conv_issue_rate_decomposed.log`, `results/aie/conv_accum_residency_npu.log` |
| **conv2dk1 peeled** (1×1 conv, int8) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop: 14 bundles (peeled n=4 case with named accs). Trip count W/4 | Loop (14 bundles): 2 `vldb`, 2 `vlda`, 0 `vst`, 2 `[x]`, 4 `[m]`, 4 `vmac` | 0.286 `vmac`/cyc (28.6%) | 73.1 MAC/cyc in loop (28.6% of 256 tile ceil) | 4 live named accs (`acc0`–`acc3`), reside in `cm0`–`cm3` | Zero in-loop stack or memory traffic; 128 B round-trips eliminated | Paired load frequency reduced; memory bus freed for input streaming | [MEASURED] `results/aie/conv_accum_residency_npu.log` |
| **conv2dk3 width-fixed** (3×3 conv, int8) | int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop (.L_LEnd10): 18 bundles (39 live ops). 14 other setup/tail loops | Loop (18 bundles): 4 `vldb`, 1 `vlda`, 0 `vst`, 14 `[x]`, 0 `[m]`, 4 `vmac` + 6 `vshift` + 4 `vmov` | 0.222 `vmac`/cyc (22.2%) | 56.9 MAC/cyc in loop (22.2% of 256 tile ceil) | 4 live 1024-bit accs in registers `cm1`–`cm4` (spill-free) | 0 in-loop spill stores; stack frame handles outer loop scalar pointers | 1 paired load in loop (0x0780 `vldb` + `vlda`); ~5.5% potential stall cycle penalty | [MEASURED] `results/aie/conv_issue_rate_decomposed.log`, `results/aie/conv2dk3_widthfix_npu.log` |
| **attention_bf16** (fused self-attention) | bf16→fp32, `aie::mul`/`mac` (16 MAC/op) | Hand-rolled 3-stage loop: QK dot-product + scalar exp Softmax + Attn·V | No HW `mmul` loops; loops serialize vector-to-scalar reductions and scalar exp | 0.000 `mmul`/cyc (uses `aie::mmul` 0 times) | 0.03–0.61 GFLOPS across array (0.0004%–0.0083% of bf16 peak) | ≤2 512-bit vector accs (`acc0`, `acc1`) in `bml` registers | Peano 1024 B default overflowed; requires `stack_size=2048` | Minimal vector pairing; bound by scalar pipeline and dispatch floor | [MEASURED] `results/aie/attention_bf16_kernel_npu.log`, `kernels/attention_bf16/README.md` |
| **int8 graph engine** (`conv_engine/engine.cc`, 4 output blocks × 2 column groups) | uint8×int8→int32, `mmul<4,8,8>` (256 MAC/op) | HW loop over input channel blocks, run-time trip `ncin` ≤ 8: stride-1 dual 23 bundles, stride-2 dual 32, single group 17. In the same object `fused_stage2`'s constant-trip loop is 9 bundles for 4 `vmac` | stride-1 dual (23 bundles): 8 weight half-loads, 4 activation half-loads, 2 `vshift` (each unaligned load is two aligned loads + `and` + `vshift`), ~9 scalar address ops, 8 `vmac`; loads then MACs, barely overlapped | 0.348 `vmac`/cyc (8 of 23); `fused_stage2` 0.444 (4 of 9, loads software-pipelined under the MACs) | Not measured on silicon for this kernel | **8 live 1024-bit accs in `cm0`–`cm7`**, names all 9; 12 vec `x0`–`x11` | 352 B frame; **0 accumulator stack moves**, 31 vector moves, 0 of either inside a hardware loop | 4 weight+activation paired loads per stride-1 dual iteration, and every core places `a*_cons_buff_1` in bank 2 beside `w*_cons_buff_0` (HAZARD: static, not timed) | [MEASURED, static] `results/aie/engine_census_widened_desktop2_20260921.log`, `engine_loops_int8_desktop2_20260921.log`, `engine_bank_check_int8_desktop2_20260921.log` |
| **bf16 graph engine core** (`bf16_conv/engine_bf16.cc`, one core, 4 output blocks × 2 column groups) | bf16→fp32, `mmul<4,8,4>` (128 MAC/op) | HW loop over input channel blocks, run-time trip: stride-1 dual 24 bundles, stride-2 dual 29, single group 18 and 21 | stride-1 dual (24 bundles): 8 weight half-loads, 6 activation half-loads, 2 `vshift.align`, 8 `vmac.f` | 0.333 `vmac.f`/cyc (8 of 24) | Not measured: this core has been checked for correctness only | 8 live 512-bit accs in `bmh0`–`bmh7` | 96 B frame; 0 accumulator stack moves, 0 vector stack moves | 6 weight+activation paired loads per stride-1 dual iteration; in the one-core harness activations sit in banks 0–1 and weights in 2–3 (SEPARATED) | [MEASURED, static] `results/aie/engine_bf16_census_desktop2_20260921.log`, `engine_loops_bf16_desktop2_20260921.log`, `engine_bank_check_bf16_desktop2_20260921.log` |

**Corrections to the table, 2026-09-23** (the rows above are kept as written):

- **int8 GEMM reblocked H11:** the "Whole-Call Rate" cell (256.0 MAC/cycle, 100%) is the *static
  in-loop* rate, not a whole-call measurement. What its log MEASURED at array scale is that the
  wall clock did not move: −2.1% best-to-best and +0.4% median-to-median against a +12% an
  issue-bound design would show, inside this machine's ~5% drift (`results/aie/gemm_reblock_h11_npu.log`, §2–§3).
- **bf16 GEMM 4×4, "SEPARATED, 0 stall":** separating A, B and C does not clear the in-loop paired
  load, because both of its loads are **B** loads and so share B's bank. With A, B and C in three separate
  banks, upstream `mm.cc` bf16's marginal cost is 32.5–33.3 cycles per inner iteration against its 32
  bundles (`results/aie/l1_tile_mm_k64_desktop2_20260923T0533Z.log`, `…_k128_…`). That is consistent with
  +1, and not isolated (1.2, cycles-per-iteration row).
- **int8 GEMM default/tuned, "A & B share Bank 2 (COLLIDE)":** the in-loop pair (`vlda wl3, [p4], #0x20 |
  vldb wl8, [p3], #0x20`) is two **A** pointers, both stepping one 4×8 A tile. It collides wherever B is
  put. Measured with A, B and C in separate banks, the loop still costs 9.8–10.2 cycles per iteration
  against 9 bundles (same logs). Bank 3 is not the fix for this pair; the nest's other nine pairs are
  not re-read here.
- **conv2dk3 width-fixed:** the log's own listing of the 18-bundle loop shows **8** `vshift` and **5**
  `vmov`, not 6 and 4 (`results/aie/conv_issue_rate_decomposed.log` §4, `0x0700`–`0x0780`). They
  issue in `[m]`, and four of them share a bundle with a `vmac` (`0x072a`, `0x075c`, `0x076c`, `0x0780`).

#### Architectural Analysis & Failure Mode Decomposition

1. **int8 GEMM: Accumulator Spilling and Loop-Nest Amortization** [MEASURED: `results/aie/gemm_cost_model_nest.log`, `results/aie/gemm_reblock_h11_npu.log`]:
   - *Inner Loop vs Whole Call*: In stock 4×2 int8 GEMM (`matmul_vectorized_4x2_mmul`), the inner hardware loop achieves an issue density of 0.889 `vmac`/cycle (8 `vmac` ops in 9 bundles), delivering 227.6 MACs/cycle (88.9% of the 256 MACs/cycle tile ceiling). However, whole-call sustained throughput collapses to 100.4–107.3 MACs/cycle (39.2%–41.9% of nameplate).
   - *The Root Cause*: Upstream selected a 4×2 accumulation grid (8 concurrently live 1024-bit accumulators). Because AIE2 possesses exactly 9 physical `cm` registers and LLVM/Peano requires registers for compiler temporaries, the spill-free ceiling is strictly ≤5 live accumulators. Holding 8 live accumulators forces LLVM to emit a 416-byte stack frame (`paddb [sp], #0x1a0`) with 33–37 stack references, creating 12 vector spill slots (12 `vlda` + 12 `vst` = 384 bytes). For each of the 8–16 accumulator groups, the core executes 87 non-loop bundles of spill save/restore and accumulator reload traffic alongside the 54 hardware loop cycles. *(Refined 2026-09-23: the spill is real, but "≤5" is the spill threshold of the probe's access pattern, not a property of the 9-register file (1.2, live-accumulator row). The spills also cost a bank stall of their own when C shares the stack's bank: 12 epilogue bundles pair a stack reload with a C store, +48.5 cycles per 32×64×32 call measured (1.2, bank row).)*
   - *The Reblocked Remedy*: Reblocking the tile to 2×2 (`matmul_vectorized_2x2_mmul` in `gemm_reblock_h11_npu.log`) reduces live accumulators to 4. This completely eliminates all 12 vector spill slots (stack frame drops from 416 B to 32 B, stack refs drop from 33 to 5), shortening the inner hardware loop to 8 bundles with 8 `vmac` ops — achieving **1.000 `vmac`/cycle (100.0% of tile ceiling, 256.0 MACs/cycle)** with zero stack traffic.
   - *Same-Bank Paired Load Penalty*: In both stock and tuned 4×2 GEMMs, the compiler places input operand buffers A (`A_L2L1_cons`) and B (`B_L2L1_cons`) into local memory Bank 2 while Bank 3 remains entirely empty. The hardware loop contains 1 paired load bundle (0x0200: `vlda wl3, [p4], #0x20 | vldb wl8, [p3], #0x20`), and the outer nest contains 9 additional paired load bundles. Because both read Bank 2 simultaneously, the hardware incurs 1 extra stall cycle per iteration, stretching the 9-cycle loop to 10 cycles and reducing effective throughput by ~4% [MEASURED: `results/aie/bank_conflict_survey.log`]. *(Refined 2026-09-23: the stall is real, 9.8–10.2 cycles per iteration measured in L1. The attribution to A and B sharing Bank 2 is not: both loads of the 0x0200 pair step `#0x20`, one 4×8 **A** tile, so they are two A loads, and the stall persisted with A, B and C in three separate banks (`results/aie/l1_tile_mm_k64_desktop2_20260923T0533Z.log`, `…_k128_…`).)*

2. **bf16 GEMM: Halved Granularity Eliminates Vector Spilling** [MEASURED: `results/aie/accumulator_width_vs_count.log`, `results/aie/bank_conflict_survey.log`]:
   - Upstream bf16 GEMM (`matmul_vectorized_4x4`) maintains a 4×4 accumulation tile (16 accumulators). In contrast to int8, bf16 accumulation uses 512-bit registers (`bml0`–`bml7` and `bmh0`–`bmh7` within `cm0`–`cm7`).
   - Although the total live accumulator footprint is identical (16 × 512 bits = 8,192 bits vs int8's 8 × 1024 bits = 8,192 bits), all 16 accumulators reside natively in the upper and lower halves of 8 physical registers. Consequently, the compiler emits **zero vector spill instructions** (`vlda`/`vst` = 0; the 64-byte stack frame contains only scalar loop control variables).
   - The steady-state hardware loop executes 16 `vmac.f` instructions in 32 bundles, yielding an issue density of 0.500 `vmac`/cycle and 64.0 MACs/cycle (50.0% of the 128 bf16 physical tile ceiling).
   - Furthermore, memory allocation assigns Bank 0 to output C, Bank 1 to input A, Bank 2 to input B, and leaves Bank 3 empty. Because input operands occupy distinct 16 KB banks, paired loads execute with zero bank conflict stalls. *(Refined 2026-09-23: separate operand banks clear A-vs-B pairs only. The loop's one paired load is two B loads, and in L1 the loop measured 32.5–33.3 cycles per iteration against 32 bundles. That is consistent with one stall per iteration, not isolated (`results/aie/l1_tile_mm_k64_desktop2_20260923T0533Z.log`, `…_k128_…`).)*

3. **conv2dk1: Register Residency vs In-Loop Memory Round-Trips** [MEASURED: `results/aie/conv_issue_rate_decomposed.log`, `results/aie/conv_accum_residency_npu.log`]:
   - *Stock Degradation*: Stock upstream `conv2dk1_i8_vector` declares accumulators in a runtime-indexed array `MMUL4x8x8 acc_tmp[4]`. Because the index `x` is variable, LLVM cannot map elements to physical `cm` registers. In the 22-bundle inner loop (.L_LEnd1), every single MAC operation requires loading four 256-bit accumulator quarters (`amhh1`, `amhl1`, `amlh1`, `amll1`), issuing 1 `vmac`, and storing four quarters back to memory (`amhh2`, `amhl2`, `amlh2`, `amll2`). Six all-nop bundles are required to cover load-to-use latencies. This produces an abysmal issue density of **0.045 `vmac`/cycle (11.6 MACs/cycle, 4.5% of tile ceiling)**.
   - *Peeled Optimization*: Peeling the primary `n == 4` execution path into four statically named `MMUL4x8x8` variables (`acc0`–`acc3`) enables the compiler to maintain accumulators directly in registers `cm0`–`cm3`. This completely eliminates all in-loop accumulator loads and stores, compressing the loop to 14 bundles issuing 4 `vmac` operations (**0.286 `vmac`/cycle, 73.1 MACs/cycle, 28.6% of tile ceiling**). On hardware, this single register-residency fix accelerates column marginal throughput from 115.6–117.1 GOPS to **348.4–350.3 GOPS (a 3.0× measured speedup)**.

4. **conv2dk3: Vector Slot Starvation by Sliding-Window Shuffling** [MEASURED: `results/aie/conv_issue_rate_decomposed.log`, `results/aie/conv2dk3_widthfix_npu.log`]:
   - In width-fixed `conv2dk3_ui8_vector`, accumulators are successfully held in registers `cm1`–`cm4` with zero in-loop spill traffic.
   - However, the 18-bundle inner loop (.L_LEnd10) achieves only **0.222 `vmac`/cycle (56.9 MACs/cycle, 22.2% of tile ceiling)** because the vector issue slot `[v]` is starved by 2D spatial convolution data alignment:
     - Out of 14 vector instructions in the loop, only 4 are `vmac` compute operations.
     - **6 bundles spend slot `[v]` on `vshift`** to realign pixels across the 3×3 filter taps.
     - **4 bundles spend slot `[v]` on `vmov`** to stage vector register operands.
   - 71.4% (10 of 14) of vector slot issues are consumed by data alignment and register shuffling rather than arithmetic, demonstrating that spatial convolution without hardware im2col or dedicated line-buffering severely degrades AIE2 VLIW compute efficiency.
   - **CONTRADICTED 2026-09-23: the mechanism above.** `vshift` and `vmov` do not spend slot `[v]`, and they do co-issue with `vmac`. SPEC(Peano) issues them in the move slot, and the same log shows `vmac` sharing a bundle with a `vshift` or `vmov` at `0x072a`, `0x075c`, `0x076c` and `0x0780` (`results/aie/peano_aie2_machine_model_a36c62b9.log` §3; `results/aie/conv_issue_rate_decomposed.log` §4). The loop's listing holds 8 `vshift` and 5 `vmov`, not 6 and 4. Realignment still costs: it occupies the `[m]` slot, adds 2-cycle latency, and forces NOP bundles wherever it sits on the critical path into a `vmac` operand. The 0.222 `vmac`/cycle stands, and a starved vector slot is ruled out as its cause. A dependency chain through the move slot is the reading left standing, but that is not separately measured.

5. **attention_bf16: Missing Matrix Engine and Scalar Bottlenecks** [MEASURED: `results/aie/attention_bf16_kernel_npu.log`, `kernels/attention_bf16/README.md`]:
   - Fused multi-head self-attention (`attention_single_head`) achieves only 0.03 to 0.61 GFLOPS across the array (0.0004% to 0.0083% of the 16-core 7.37 TFLOPS bf16 ceiling), trailing a Zen4 CPU by 71×–240×.
   - *Design Defect*: The kernel issues `aie::mmul` **zero times**. Rather than computing 2D matrix blocks, the inner loop hand-rolls row-wise dot products using 16-lane vector multiplies (`aie::mul`) followed immediately by horizontal vector reductions (`aie::reduce_add<float>`). This serializes vector execution and forfeits the native 128 MACs/cycle bf16 matrix engine.
   - *Scalar Contention*: Softmax evaluation executes scalar polynomial approximation (`fast_exp`) and scalar sum reductions in loop nests, leaving the 6-slot VLIW issue pipeline largely idle.
   - *Stack Hazard*: The default Peano stack limit of 1024 bytes grew upward into tile data buffers, causing silent memory corruption until resolved by explicitly passing `Worker(..., stack_size=2048)`.

### 2.3.2 Schedule without data movement: hand vs compiled, and the shipped engine core (2026-09-23)

Every number above except the static ones includes data movement, so none of them could say how much
of a gap is *schedule*. `tools/l1_tile_bench.py` removes movement. The operands sit in L1, the kernel
is called 10⁵–10⁶ times per dispatch, and time is fitted against the call count, so dispatch and DMA
fall into the intercept (R² ≥ 0.9992 on every fit). Every output is checked exactly against numpy.
An empty-call control prices the harness at 19.1–20.4 cycles per call. MEASURED, Desktop 2,
`default` pmode, 1.80 GHz:

| Kernel, tile | cycles/call | over the control | % of per-core peak | Log (`results/aie/`) |
|---|---|---|---|---|
| *Hello XDNA!* hand `.s`, bf16 32³ | 301.3 | **282.0** | **85.0%** (391.5 GFLOPS) | `l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log` |
| upstream `mm.cc`, bf16 32³ (Peano `-O2`) | 898.6 | 879.3 | 28.5% | same sitting |
| upstream `mm.cc`, int8 32³ | 533.4 | 514.1 | 24.0% | same sitting |
| `mm.cc` bf16, 32×64×32 / 32×128×32 | 1418.5 / 2482.4 | — | 36.1% / 41.3% | `l1_tile_mm_k64_…0533Z.log`, `l1_tile_mm_k128_…0533Z.log` |
| `mm.cc` int8, 32×64×32 / 32×128×32 | 697.1 / 1009.4 | — | 36.7% / 50.7% | same two |
| **`mm.cc` bf16, 64³** (the array GEMM's tile) | 5427.4 | 5407.0 | **37.7%** (173.9 GFLOPS) | `l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log` |
| `mm.cc` int8, 64³ | 2604.8 | 2584.4 | 39.3% (362.3 GOPS) | same |

- **A hand schedule runs exactly as written.** 282.0 = the kernel's 288 executed bundles less the
  control's 6 executed bundles (`ret lr` + 5 delay slots; the control's `.o` has 7 static bundles, the
  seventh a `.p2align` pad), so zero stalls. On an identical tile, harness and bank placement it is
  **3.0× the compiled upstream kernel** (282 vs 879, bf16 32³). The comparison that matters for the array
  is hand 85% at 32³ against compiled 37.7% at 64³. The reference kernel's output is exact
  (C0 + calls·A@B in `mm.cc`'s tiled layout), which the reference itself never checked.
- **The compiled kernel's ceiling is its schedule.** Its bf16 loop is 16 `vmac.f` in 32 bundles (a 50%
  cap) plus one same-bank paired load per iteration, and each output block reloads and stores its C
  accumulators. Longer K amortises only the last of these (1.2, cycles-per-iteration row).
- **Placement can cost a compiled call 7%:** `mm.cc` int8 with C beside the stack (1.2, bank row).

**The shipped engine core, static** (`results/aie/engine_core_issue_census_desktop2_20260923.log`,
`tools/engine_core_census.py`, all 68 `build/conv_engine/` builds, 4 distinct core ELFs): its conv inner
loops issue **0.167–0.348 `vmac` per cycle** (fused stage 2: 0.444), with every build's conv loops
in that range. Each iteration does loads, then `load_unaligned_v` realignment, then idles, then issues
8 `vmac`s, with no overlap across iterations. A bundle-count reading, not a silicon run. Against the
hand kernel's 89% `vmac` issue on the same silicon, the engine core's inner loops are the largest
untested lever in this repo. The static bank check reports a HAZARD: 12 paired loads in loop bodies,
with activations and weights sharing bank 2.
...
**Attributed 2026-09-23: the remaining two thirds are the kernel.** Upstream `mm.cc` bf16 at the
array's own 64×64×64 tile, run with its operands already in L1 and no movement at all, reaches
**37.7% of the per-core peak** (173.9 GFLOPS, 5,427.4 cycles per call). MEASURED
`results/aie/l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log`, 2.3.2. Sixteen such
cores make 2,782.4 GFLOPS (DERIVED, 16 × 173.9). The array's measured 64/64/64 figures at 2048³ and above run from 2,477.23 GFLOPS
(2048³, `bf16_matmul_n64_single_buffer_npu.log`) to 2,653.05 (2048×4096×4096,
`gemm_tile_sweep_c_single_buffer_npu.log`), which is 33.6–36.0% of 7,372.8. They are therefore
**89–95% of what the kernel can do in L1**. 1024³ sits below that at 2,080.98, the small-shape
caveat every GEMM here carries. Dispatch and input delivery together account for at
most a few points of peak. The "K-loop's C read-modify-write" is inside the kernel, not beside
it: `mm.cc` reloads and re-stores its 16 C accumulators once per output block. The kernel's
own losses are its 16-in-32-bundle loop (a 50% cap), one same-bank paired load per iteration,
and that per-block C traffic (1.2, 2.3.2). The int8 array at this tile (4,607.05 GOPS, 31.2%
at 1.80 GHz) reaches about 79% of its kernel's 39.3%, so int8 is the more movement-bound of the
two. That is consistent with H11's reblock not moving the int8 wall clock
(`results/aie/gemm_reblock_h11_npu.log`).
...
trace in S2 apportions those. *(Refined 2026-09-23: the core-side half now has a measured ceiling.
At k=64 `mm.cc`'s per-block cost does not depend on m or n, and in L1 it reaches 36.1% at
32×64×32 and 37.7% at 64³, so that kernel caps any k=64 tile near 37% of peak whatever the
data movement does. The default 64×32 tile was not run in L1. The paragraph above attributes the
64×64 case.)* Second, the levers the silicon offers are specific: keep the
...
spill-free ceiling *(refined 2026-09-23: 5 is that probe's vector-register-pressure threshold, not
the 9-register file's; 1.2)*. **The computed rate is now checked on silicon:** the same kernel at
64³ with its operands in L1 measured 39.3% of peak, against 40.3–41.9% computed
(`results/aie/l1_tile_mm_direct_64x64x64_desktop2_20260923T0538Z.log`, 1.2). **Unstarted:** the decimating hardware counter — subclass
...
*(Refined 2026-09-23: the log's listing of that loop holds 8 `vshift` and 5 `vmov`, not six and four.
They issue in the move slot, not the vector slot, and four of them share a bundle with a `vmac`
(2.3.1). "Spent in issue slots" should read "spent on the move slot and its 2-cycle latency on the
path into each `vmac`". Moving them to the mem tile's BDs would still remove that dependency, and
a hand schedule is now a demonstrated alternative route (2.3.2).)*
...
**Re-scoped 2026-09-23: K2's bar cannot be reached with this kernel.** Upstream `mm.cc` bf16 at
64³ reaches 37.7% of peak with *no* data movement (2.3.2, 3.1). The array's 33.6–36.0% (2048³ and above) is already
89–95% of that, so no DMA, tile or dispatch work moves it much. The lever is the core schedule.
A hand-scheduled bf16 tile runs at 85.0% on this silicon in the same harness (2.3.2;
`results/aie/l1_tile_compiler_vs_hand_i32loop_desktop2_20260923T0530Z.log`). A kernel at that
rate, fed at the 64×64 tile's 100% input bound, is what K2 now needs. Whether its 32³ shape keeps
that rate at 64×64×64 is untested.
