...
The persistent conv-engine core (`kernels/aie2/conv_engine/engine.cc`) is one program serving every packet, opcode selected by header word 0. The spatial-stencil-fusion work already on this branch left it at **13,424 B of the 16,384 B program memory** (docs/DECISIONS.md) — **2,960 B of headroom**, not the 7-11 KB the pre-fusion census implied. *(Superseded 2026-09-21: 13,424 B is the kernel object, not the linked core. IRON's core wrapper adds a constant 880 B, so the headroom then was 2,080 B — see [the linked-size section](#the-census-prices-the-kernel-object-and-the-core-holds-880-b-more-the-int8-engine-has-224-b-free-2026-09-21-desktop-2).)* Three non-attention tensor ops that today survive only as host-segment ONNX Runtime calls or as fusions into conv epilogues were implemented as opcodes under that budget, priced individually before the combination was landed, and all three fit:
...
| all three | final committed program | **15,280 B** | **+1,856 B**, 1,104 B still free *(superseded 2026-09-21: 224 B free once linked, [below](#the-census-prices-the-kernel-object-and-the-core-holds-880-b-more-the-int8-engine-has-224-b-free-2026-09-21-desktop-2))* |
...
## The census prices the kernel object, and the core holds 880 B more: the int8 engine has 224 B free (2026-09-21, Desktop 2)

Backing log: [`results/aie/engine_linked_program_size_desktop2_20260921.log`](../results/aie/engine_linked_program_size_desktop2_20260921.log). Tool: `tools/engine_linked_size.py` — `llvm-size -A` and `llvm-objdump -t` on builds that already exist. Static: nothing was compiled and the NPU was not opened, so this establishes a size and no latency.

`tools/engine_opcode_census.py` compiles `engine.cc` alone and sums its `.text`. Program memory holds the **linked core**: that object plus the core program IRON generates around it. Read off eight existing builds, all sixteen cores of each agreeing:

| Build | kernel object `.text` | linked core `.text` | free of 16,384 B |
|---|---:|---:|---:|
| `yolov8n_full`, `yolov8s_opt`, `resnet50_head` (2026-09-17) | 9,824 B | 10,704 B | 5,680 B |
| `yolov8n_rb2`, `sesr_m7`, `bisenetv2_dense_20260921`, `modnet_cut_dense_20260921` | **15,280 B** | **16,160 B** | **224 B** |
| `yolov8n_stride2ctrl` (an experiment's kernel, never documented) | 15,488 B | 16,368 B | 16 B |
| bf16 single-core harness (`kernels/bf16_conv/engine_bf16.py`) | 3,520 B | 4,048 B | 12,336 B |

The difference is **880 B in every 16-core build, across three different kernel objects**, and the ELF's own symbol table itemises it: `core_<c>_<r>` 768 B (the ObjectFifo acquire/release loop of `design.py::_core_fn`) + `_main_init` 80 B + `__start` 32 B. The bf16 harness's simpler core loop makes its wrapper 528 B (416 + 80 + 32). `OP_POOL`'s marginal cost in the section above is also +880 B; that is a coincidence, not the same bytes.

- **What this corrects.** "15,280 B of 16,384 B, 1,104 B still free" is true of the object and was read as true of the core. The committed engine has **224 B** of program memory left, and the section above's 13,424 B point had 2,080 B, not 2,960 B. Both are marked superseded in place.
- **The budget a census must meet.** Under this wrapper an `engine.cc` object may be at most **15,504 B**. A census row of, say, 16,300 B reads as "under 16,384" and would not link. The `OP_POOL`-without-pragmas row above (16,464 B) was over the object budget by 960 B, not by 80 B.
...
> **Superseded 2026-09-21, same day, by [the section that follows](#the-bf16-hot-loop-rewrites-do-reach-the-wall-clock-10-and-18-percent-off-a-k3-pass-and-the-earlier-sitting-timed-one-kernel-under-five-names-2026-09-21-desktop-2).** The device half of this section is void: the jit keys a design before its generator runs, so the kernel source chosen inside the generator was not in the cache key, and the four `--also` arms and the `chunk`/`hoist+ptr` sweeps ran cached designs of other sources ([`engine_bf16_cache_audit_desktop2_20260921.log`](../results/aie/engine_bf16_cache_audit_desktop2_20260921.log) names the source each cache entry was built from). The static table stands. The timing table, the "H11 again" reading, the k1 paragraph and both closing bullets do not. Kept as written, per the rule that a recorded number is marked, never deleted.
...
## The bf16 hot-loop rewrites do reach the wall clock, 10 and 18 percent off a k3 pass, and the earlier sitting timed one kernel under five names (2026-09-21, Desktop 2)

Backing logs: [`engine_bf16_cache_audit_desktop2_20260921.log`](../results/aie/engine_bf16_cache_audit_desktop2_20260921.log) (the cache before the reruns, static) and [`..._02.log`](../results/aie/engine_bf16_cache_audit_desktop2_20260921_02.log) (after); byte-exactness with the object named in each log, `--checks-only --npu`: [`chunk`](../results/aie/engine_bf16_loop_variants_exact_chunk_npu_20260921.log), [`hoist`](../results/aie/engine_bf16_loop_variants_exact_hoist_npu_20260921.log), [`hoist+ptr`](../results/aie/engine_bf16_loop_variants_exact_hoist_ptr_npu_20260921.log), [`ptr`](../results/aie/engine_bf16_loop_variants_exact_ptr_npu_20260921.log); timing, `timing_eligible: true`, host CLEAR, alternating arms in one process: [`bench_k3_npu_20260921_02`](../results/aie/engine_bf16_loop_variants_bench_k3_npu_20260921_02.log), [`bench_k1_npu_20260921_02`](../results/aie/engine_bf16_loop_variants_bench_k1_npu_20260921_02.log). Tools: `tools/engine_bf16_cache_audit.py` (new), `kernels/bf16_conv/engine_bf16.py` (the source is now a `CompileTime` value of the design, and every design prints an `ENGINE_BF16_DESIGN` line with its cache entry and the `.text` size of the object it linked). `engine_bf16.cc` and `engine.cc` are still unchanged.

**What was wrong.** `@iron.jit` computes a design's cache key from the generator's bytecode, its `CompileTime` values and the decorator's `source_files` list, *before* the generator body runs (`aie/utils/compile/jit/_hash.py`). The harness chose the kernel source inside the body, from a module global, so two sources with the same `(packets, repeat)` had the same key, and the second silently ran the first's xclbin. The cache audit, which names the source each entry compiled from by digest against the variant copies, shows what the earlier sitting ran: the generator's bytecode changed in that commit, so the first run after it (the `ptr` sweep) built fresh `repeat=1` entries from `ptr`; the first timing arm then built the only `repeat=128` entry, from the committed kernel. So in both earlier timing logs every engine arm's *single-dispatch* design was `ptr` and every *amortised* design was the committed kernel, and the `chunk` and `hoist+ptr` "byte-exact" sweeps ran `ptr`. Five arms, one kernel: the table could not have moved. The harness now refuses to time two arms that resolve to one cache entry.

**Byte-exactness, with the object in the log.** Each variant's sweep now carries its own `ENGINE_BF16_DESIGN` lines - `chunk` 6,192 B, `hoist` 3,456 B, `hoist+ptr` 3,552 B, `ptr` 3,600 B, each in two fresh cache entries - and each is 1,600 of 1,600 on all ten sweep cases (including the in-dispatch `F_LOAD_PSUM` chains, k5 and the 8-block k1) and reproduces the committed kernel's accumulate probe exactly (aligned 1,600/1,600 on both sets; the rejected models score what they scored before). `hoist` had never been swept before.

**On the device, with ten distinct engine designs alive in one process.** Dispatch-free microseconds per pass, the slope between 1 and 128 in-core repeats, medians of five alternating rounds; the range is the five rounds' spread:

| arm | object | k3, 4 input blocks (460,800 MACs) | k1, 8 input blocks (102,400 MACs) |
|---|---:|---:|---:|
| base (the committed kernel) | 3,520 B | 10.277 us (10.12-10.46) | 3.310 us (3.26-3.42) |
| ptr | 3,600 B | 9.238 (**-10.1%**; 9.10-9.38) | 3.094 (-6.5%; 3.07-3.20) |
| chunk | 6,192 B | **8.460** (**-17.7%**; 8.32-8.61) | **2.858** (-13.7%; 2.79-2.99) |
| hoist | 3,456 B | 10.093 (-1.8%; 10.04-10.16, overlaps base) | 2.988 (-9.7%; 2.95-3.12) |
| hoist+ptr | 3,552 B | 9.173 (-10.7%; 9.09-9.32) | 2.969 (-10.3%; 2.90-3.01, one round at 4.03) |
| `conv_bf16.cc`, milestone 1 | - | 9.165 (-10.8%; 9.14-9.51) | 2.131 (-35.6%; 2.03-2.19) |
...
- **What it decides for int8:** the port to `engine.cc` was gated on a bf16 win and there is one. The `ptr` pattern cost +80 B on bf16 against the int8 core's 224 B free once linked; `chunk`'s +2,672 B cannot land without gating dispatch groups no container reaches (5,456 B priced). See `docs/DECISIONS.md`.
- **Rule this adds:** a harness that builds kernels through `@iron.jit` puts the kernel source in the design's `CompileTime` values or `source_files`, never in a global read by the generator, and every timing log names the object each arm ran (`ENGINE_BF16_DESIGN`). A log that says which source it *meant* to build has said nothing about which kernel ran.
