...
- **The win is placement, not speed.** The Vitis AI EP cannot place a terminal Gemm on Phoenix silicon — the same wall that made detection users ship `yolov8n_cut.onnx` — so in AMD's stack a classifier has no NPU path at all; the engine runs the identical head wholly on the device with 0 host segments. For the head alone AMD's own CPU fallback is faster and leaner, and this table says so: a lone 2,048 x 1,000 Gemm is small work for a CPU. The NPU path earns its keep only with the head sitting beside a backbone on the device, and no whole-classifier container has been built or sat.
