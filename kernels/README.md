...
|---|---|---|
| [aie2/conv_engine](aie2/conv_engine/design.py) | The whole YOLOv8n graph (63 convs, 3 SPPF pools): one persistent packet-driven program on 16 cores, weights streamed per packet, activations in a DDR workspace. The same program runs YOLOv8s, SESR M7, YOLOv8n-pose, YOLO11n and YOLO-World v2, the last two with host segments between NPU dispatches | **Every layer bit-exact on Device 0** (and offline against ONNX Runtime); 66-layer dispatch 7.2 ms with the coarse schedule and 7.9 ms witnessed glass-to-glass. [Evidence and limits](../docs/BENCHMARKS.md#whole-network-yolov8n-on-a-16-core-convolution-engine-every-layer-on-the-npu-bit-exact-2026-09-13-desktop-2). Two opt-in MemTile structures, an activation ring and a resident weight buffer, are byte-exact and slower, and stay off ([evidence](../docs/BENCHMARKS.md#memtile-residency-does-not-pay-on-the-graph-engine-and-the-yolov8s-gap-is-a-known-limitation-2026-09-16-desktop-2)). Header flags: 32, residual operand shifts; 4 on a residual packet (2026-09-16), HardSwish after the add, exact and no slower on YOLOv8n but used by no model ([evidence](../docs/BENCHMARKS.md#yolo-world-v2-with-only-its-text-attention-on-the-cpu-gptq-and-an-int32-bias-recover-the-four-output-convolutions-2026-09-16-desktop-2)); 64 (2026-09-17), SiLU through a four-line sigmoid applied after the passes to the finished tile, lines 2-4 in header words 26-31, set only by `--silu-sigmoid`: exact on the NPU, 2.4-3.9 % more dispatch time, and no measurable cost to packets without it ([evidence](../docs/BENCHMARKS.md#the-sigmoid-silu-epilogue-on-the-npu-an-opt-in-exact-through-the-containers-for-24-39--more-dispatch-time-2026-09-17-desktop-2)). Biases are int32 per output channel; the output shift is one per packet |
...
using 8 `MMUL4x8x8` accumulators on hardware that only has 6 accumulator registers *(superseded
2026-09-23: the file is 9 × 1024-bit `cm0`–`cm8`, SPEC(Peano)
[`peano_aie2_machine_model_a36c62b9.log`](../results/aie/peano_aie2_machine_model_a36c62b9.log) §1.
The engine's bit-exact ELFs name `cm8`, [ledger A1](../results/aie/notes_tnzr_cross_audit.md). Eight
accumulators fit in nine, so "only 6" cannot be the cause of the spill below. Its cause is not
re-derived here)*. Peano
