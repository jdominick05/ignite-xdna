...
|---|---|---|
| [aie2/conv_engine](aie2/conv_engine/design.py) | The whole YOLOv8n graph (63 convs, 3 SPPF pools): one persistent packet-driven program on 16 cores, weights streamed per packet, activations in a DDR workspace | **Every layer bit-exact on Device 0** (and offline against ONNX Runtime); 66-layer frame 11.7 ms with the committed stream, so the 8 ms target is open. [Evidence and limits](../docs/BENCHMARKS.md#whole-network-yolov8n-on-a-16-core-convolution-engine-every-layer-on-the-npu-bit-exact-2026-09-13-desktop-2) |
