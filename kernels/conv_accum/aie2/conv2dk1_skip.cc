...
// now, we do not.
//*****************************************************************************
// Local fix (2026-09-07, this checkout only): upstream's remainder path here
// was never even implemented (a commented-out stub below), and iw_32_rem was
// hardcoded to 0 -- so any input_width not a multiple of 32 silently dropped
// its remainder columns from output AND from the skip-add. Rewritten to walk
// the width in blocks of N<=4 chunks, addressed directly from base pointers
// (no incremental pointer state), keeping live accumulators within AIE2's 6
// hardware accumulator registers (upstream used 8). Weight layout unchanged:
// kernels are walked sequentially through the full (input_channels/8) range
// per oc/8 block (input0's half first, then input1's half), matching the
// original {OC}{IC}{IC8}{OC8} layout. See conv2dk1_i8_vector / conv2dk3.cc's
// own fix for the same pattern and its empirical verification.
void conv2dk1_skip_i8_vector(uint8_t *input0, uint8_t *input1, int8_t *kernels,
  ...
  const int iw = input_width;
#endif
  const int ics16 = input_channels / 16; // per-half (input0/input1) ic/8 count
  const int ocs8 = output_channels / 8;
  const int wts_per_oc = (input_channels / 8) * 64; // both halves, per oc/8
  ...
  for (int oc = 0; oc < ocs8; oc++) {
    ...
    while (chunk < total_chunks) {
      ...
      // ACCUMULATOR RESIDENCY FIX -- see the identical change in this
      // directory's conv2dk1.cc for the full reasoning. `acc_tmp` is an array
      // indexed by a loop whose trip count `n` is a RUNTIME value, so it cannot
      // live in registers and every `.mac()` becomes load-four-quarters / mac /
      // store-four-quarters. This peels the n == 4 case into four NAMED
      // accumulators; the array loop below is kept as the tail and is genuinely
      // reached for widths whose total_chunks is not a multiple of 4 -- iw=56
      // gives 14 = 4+4+4+2, which the 56x56 sweep shape exercises and verifies.
      //
      // This kernel is the bottleneck design's THIRD pipeline stage. Patching
      // only conv2dk1.cc (stage 1) leaves the pipeline rate set by whichever
      // stage is slowest, which is why both 1x1 stages are fixed together.
      if (n == 4) {
        ...
        int8_t *k_ptr = kernels + (size_t)oc * wts_per_oc;

        for (int ic = 0; ic < ics16; ic++) {
          aie::vector<int8, 64> in_b = aie::load_v<64>(k_ptr);
          k_ptr += 64;
          const uint8_t *a_ptr =
              input0 + (size_t)ic * iw * 8 + (size_t)chunk * 32;
        ...
        for (int ic = 0; ic < ics16; ic++) {
          ...
          k_ptr += 64;
          const uint8_t *a_ptr =
              input1 + (size_t)ic * iw * 8 + (size_t)chunk * 32;
          C0.mac(aie::load_v<32>(a_ptr + 0 * 32), in_b);
          C1.mac(aie::load_v<32>(a_ptr + 1 * 32), in_b);
          C2.mac(aie::load_v<32>(a_ptr + 2 * 32), in_b);
          C3.mac(aie::load_v<32>(a_ptr + 3 * 32), in_b);
        ...
        const int8_t *s_base = skip + (size_t)oc * iw * 8 + (size_t)chunk * 32;
        uint8_t *o_base = output + (size_t)oc * iw * 8 + (size_t)chunk * 32;
#define SKIP_STORE(IDX, ACC)                                                   \
  {                                                                            \
    aie::vector<int8, 32> skip1 = aie::load_v<32>(s_base + (IDX) * 32);         \
    aie::accum<acc32, 32> accj;                                                \
    accj.from_vector(skip1, 0);                                                \
    accj = aie::add(accj, (ACC).to_vector<int8>(scaleT));                      \
    aie::store_v(o_base + (IDX) * 32, accj.to_vector<uint8>(skip_scaleT));     \
  }
        SKIP_STORE(0, C0)
        SKIP_STORE(1, C1)
        SKIP_STORE(2, C2)
...
#undef SKIP_STORE
      ...
      for (int ic = 0; ic < ics16; ic++) {
        ...
        for (int x = 0; x < n; x++) {
          const uint8_t *a_ptr =
              input0 + (size_t)ic * iw * 8 + (size_t)(chunk + x) * 32;
      ...
      for (int ic = 0; ic < ics16; ic++) {
        ...
        for (int x = 0; x < n; x++) {
          const uint8_t *a_ptr =
              input1 + (size_t)ic * iw * 8 + (size_t)(chunk + x) * 32;
          aie::vector<uint8, 32> in_a = aie::load_v<32>(a_ptr);
          acc_tmp[x].mac(in_a, in_b);
        }
      }
      ...
      for (int x = 0; x < n; x++) {
        const int8_t *s_ptr =
            skip + (size_t)oc * iw * 8 + (size_t)(chunk + x) * 32;
        aie::vector<int8, 32> skip1 = aie::load_v<32>(s_ptr);

        aie::accum<acc32, 32> accj;
        accj.from_vector(skip1, 0);
        accj = aie::add(accj, acc_tmp[x].to_vector<int8>(scaleT));
