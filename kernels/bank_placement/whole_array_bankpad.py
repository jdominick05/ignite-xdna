...
def _make_argparser():
    ...
    p.add_argument(
        "--stack-size",
        default="0xD00",
        help="per-core stack size. The allocator lays local buffers out ABOVE "
             "the stack, so raising this shifts every buffer up by the same "
             "amount and is a pure placement change: same kernel, same DMA, "
             "same schedule, same compiled function bytes.",
    )
    p.add_argument(
        "--c-single-buffer",
        type=int,
...
def _compile_kwargs(opts):
    return dict(
        ...
        stack_size=int(opts.stack_size, 0),
    )
