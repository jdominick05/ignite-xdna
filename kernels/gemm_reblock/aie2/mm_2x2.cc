...
static inline void matmul_vectorized_4x8x8_i8_i32(const int8 *__restrict pA,
  ...
  // REBLOCKED FOR THIS EXPERIMENT (H11). Upstream dispatches int8->int32 to
  // matmul_vectorized_4x2_mmul, which holds EIGHT live 4x8x8 accumulators. This
  // repo measured five as the spill-free ceiling and the resulting object spills
  // twelve vector values into a 416-byte frame, while the bf16 path's blocking
  // spills nothing at all (results/aie/accumulator_width_vs_count.log). The 2x2
  // template holds four, under the ceiling. It steps m by 2 rather than 4, so the
  // divisibility precondition relaxes to 2*r.
  static_assert(m % (2 * r) == 0);
  static_assert(k % s == 0);
  static_assert(n % (2 * t) == 0);
