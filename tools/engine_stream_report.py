...
def report(model: Path) -> dict:
    ...
    for s in scheds:
        ...
        for prog in s.programs:
            for it in prog:
                ...
                per[kind]["bytes"] += sum(p.nbytes for p in patterns(it[1:]))
        for k in per:
    ...
    return {"model": model.name, "layers": len(scheds), "rounds": sum(s.rounds for s in scheds),
            "static_weight_packet_bytes": store.nbytes, "activation_packets": packets,
            "activation_bytes_check": totals["activation"]["bytes"] == packets * A_BYTES,
