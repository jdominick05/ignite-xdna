...
Detect-head egress layout for YOLOv8 containers.
...
class DetectHeadLayout:
    """Resolved placement of the six heads inside one egress buffer."""
    ...
    def unpack(self, raw: np.ndarray) -> Dict[str, np.ndarray]:
        """Return the six heads as int8 views of ``raw`` (no copy, no dequantization)."""
        flat = np.asarray(raw)
...
def declared_head_bytes(manifest: Mapping[str, Any]) -> int:
    """Total int8 bytes of the six heads named in ``output_shapes`` (0 if any is missing)."""
...
def resolve_head_layout(manifest: Optional[Mapping[str, Any]], egress_bytes: int) -> HeadStatus:
    """Decide whether ``egress_bytes`` of device output carry the six raw heads.

    Fails closed: every check below must pass for ``present`` to be True.
    """
    manifest = manifest or {}
    egress_bytes = int(egress_bytes)
    declared = declared_head_bytes(manifest)
    shapes = manifest.get("output_shapes") or {}
    ...
    if declared == 0:
        return _absent("manifest output_shapes does not name all six detect heads", declared, egress_bytes)
    for name in HEAD_NAMES:
    ...
    return HeadStatus(present=True,
                      reason=f"six heads declared in head_layout, {layout.required_bytes} of "
                             f"{egress_bytes} egress bytes",
                      declared_bytes=declared, egress_bytes=egress_bytes, layout=layout)


def contiguous_head_layout(shapes: Mapping[str, Any], scales: Mapping[str, Tuple[float, int]]) -> Dict[str, Dict[str, Any]]:
