...
def build_manifest(ir: GraphIR, ws: es.Workspace, scheds: List[es.LayerSchedule], store: es.PacketStore,
    ...
    return {
        ...
        "producer": "ignite-compile v0.3.0 (graph engine)",
        ...
        "quant_scales": {"input_scale": ir.tensors[ir.input].scale, "input_zero_point": ir.tensors[ir.input].zero_point,
                         "input_dtype": "uint8"},
        "graph_engine": {
...
def compile_graph_container(onnx_path, output_path, build_dir: Optional[Path] = None, layers: Optional[int] = None,
    ...
    def body(ws_arg, wp_arg):
        emitter = SequenceEmitter(ws_arg, wp_arg, eng.WS_BYTES, eng.WP_BYTES,
    ...
    program = eng.build_program(iron.get_current_device(), body)
